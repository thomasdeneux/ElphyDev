...
extern "C" {  CUDA1_API HRESULT TestTexture(); }
extern "C" {  CUDA1_API HRESULT initDX( HWND hWnd, IDirect3D9 **IDX, IDirect3DDevice9Ex  **IDevice); }
...
HRESULT initDX( HWND hWnd, IDirect3D9 **IDX, IDirect3DDevice9Ex  **IDevice)
{
   HRESULT res = InitD3D9(hWnd);

   *IDX = g_pD3D;

   *IDevice = g_pD3DDevice;
...
HRESULT RegisterD3D9ResourceWithCUDA()
{
	...
	g_texture_2d.pTexture->AddRef();
	int refC = g_texture_2d.pTexture->Release();
...
HRESULT InitTextures()
{
    ...
    // 2D texture
    g_texture_2d.width  = 728;
    g_texture_2d.height = 421;
