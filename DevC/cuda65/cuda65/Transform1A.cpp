...
{               
        ...
        CartToPolK1( LinearMem1, LinearMem2, width1, height1, PitchMem1, BK );     
...
void DispSurfaceOnSurface(void *surf1, int pitch1, int Nx1, int Ny1, 
                          ...
                          int AlphaMode, int LumMode, int *Lum, int *Alpha,
                          int Mask);  
...

{
  ...
  res = cudaMalloc( (void**)&Lum, 4*sizeof(int) );
  res = cudaMemcpy( Lum , Lum1 ,4*sizeof(int), cudaMemcpyHostToDevice);

  res = cudaMalloc( (void**)&Alpha, 4*sizeof(int) );
  res = cudaMemcpy( Alpha , Alpha1 ,4*sizeof(int), cudaMemcpyHostToDevice);
...
int Ttransform::SmoothSurface( int N1, int N2, int x0, int y0, int dmax, int dmax2, int* ref)
{
  ...
  //test
  //cudaMallocPitch(&LinearMemDum, &PitchMemDum, width1*4, height1);
  //end Test
  ...
  //SmoothSurf2(LinearMem1, LinearMem2, LinearMemDum, PitchMem2, width2, height2, N1, N2,x0,y0,dmax, dmax2,ref);  // test
  SmoothTex2(cuArraySrc, LinearMem2, LinearMemDum, PitchMem2, width2, height2, N1, N2, x0,y0,dmax, dmax2,ref);   
...
int Ttransform::PixSum(int comp, int ref)
{
  ...
  res = TexSum( SrcArray , width1, height1, NULL, comp, ref );
