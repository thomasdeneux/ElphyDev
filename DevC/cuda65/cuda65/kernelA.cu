...
extern int MaxDimBlock1;
extern int MaxDimBlock2;
...
__global__ void KDispSurfaceOnSurface(unsigned char *surf1, int pitch1, int Nx1, int Ny1, 
                                     ...
                                     int AlphaMode, int LumMode,  int *Alpha, int *Lum,
                                     int Mask)
{
...
void DispSurfaceOnSurface(void *surf1, int pitch1, int Nx1, int Ny1, 
                          ...
                          int Mask)                          
{
  dim3 Db =  dim3(MaxDimBlock2, MaxDimBlock2);                 
  dim3 Dg = dim3((Nx2+Db.x-1)/Db.x, (Ny2+Db.y-1)/Db.y);

  KDispSurfaceOnSurface<<<Dg,Db>>>((unsigned char*)surf1, pitch1, Nx1, Ny1, 
                                     ...
                                     int Mask)
{
    ...
    if ((AlphaMode==3) && (pixel2[3]==Alpha[0]))
    {
      pixel2[3]= pix.w*(255.0-Alpha[0])/255.0 + Alpha[0];
    }
                          ...
                          int Mask)                          
{
    ...
    KDispTexOnSurface1<<<Dg,Db>>>( Nx1, Ny1, 
                        (unsigned char*)surf2, pitch2, Nx2, Ny2, 
                        x0, y0,theta, ax, ay, 
                        xcSrc, ycSrc, xcDest, ycDest,
                        AlphaMode, LumMode, Alpha, Lum, Mask );
                               ...
                               int N1, int N2, int x0, int y0, int dmax, int dmax2, int* ref )
{
    ...
    KSmoothTexCol<<<Dg,Db>>>( (unsigned char*)surfDum, pitch, Nx, Ny, N1, N2, x0, y0, dmax);
