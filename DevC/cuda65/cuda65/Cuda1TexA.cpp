...

{
  ...
  res = cudaMalloc( (void**)&Lum, 4*sizeof(char) );
  res = cudaMemcpy( Lum , Lum1 ,4*sizeof(char), cudaMemcpyHostToDevice);

  res = cudaMalloc( (void**)&Alpha, 4*sizeof(char) );
  res = cudaMemcpy( Alpha , Alpha1 ,4*sizeof(char), cudaMemcpyHostToDevice);
