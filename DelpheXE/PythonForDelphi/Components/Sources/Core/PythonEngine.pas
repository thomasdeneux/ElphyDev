...
//* XXX the ordering here is weird for binary compatibility */
  T_CHAR                        = 7;	//* 1-character AnsiString */
...
type
   TPChar  = array[0..16000] of PansiChar;
   PPansichar  = ^TPChar;
   PInt	   = ^Integer;
   PDouble = ^Double;
   PFloat  = ^Real;
...
{$ENDIF}
  ...
  PyClassObject = packed record
    ...
    cl_name    : PPyObject;       // A AnsiString
...
{$ENDIF}
    ...
    co_filename    : PPyObject;       // AnsiString (where it was loaded from)
    co_name        : PPyObject;       // AnsiString (name, for reference)
    co_firstlineno : Integer;         // first source line number
    co_lnotab      : PPyObject;       // AnsiString (encoding addr<->lineno mapping)
  end;
...
#define _PyTZINFO_HEAD		\
	PyObject_HEAD		\
	long hashcode;		\
	AnsiChar hastzinfo;		/* boolean flag */
}
 ...
 *}
  _PyDateTime_BaseTZInfo = packed record
    ...
    // End of the Head of an object
    hashcode   : Integer;
    hastzinfo  : AnsiChar;  // boolean flag
...
{#define _PyDateTime_TIMEHEAD	\
	_PyTZINFO_HEAD		\
	unsigned AnsiChar data[_PyDateTime_TIME_DATASIZE];
}
...
#define _PyDateTime_DATETIMEHEAD	\
	_PyTZINFO_HEAD			\
	unsigned AnsiChar data[_PyDateTime_DATETIME_DATASIZE];
}
...
type
  ...
  private
    ...
    procedure SetDllName(const Value: AnsiString);
  protected
    FDllName            : AnsiString;
    FDllPath            : AnsiString;
    FAPIVersion         : Integer;
    FRegVersion         : AnsiString;
    FAutoLoad           : Boolean;
    FAutoUnload         : Boolean;
    ...
    function  GetQuitMessage : AnsiString; virtual;
  ...
  published
    ...
    property DllName : AnsiString read FDllName write SetDllName stored IsDllNameStored;
    ...
    property RegVersion : AnsiString read FRegVersion write FRegVersion stored IsRegVersionStored;
...
type
  ...
  protected
    ...
    function  GetQuitMessage : AnsiString; override;
    procedure CheckPython;
    function  GetUnicodeTypeSuffix : AnsiString;
...
type
  TEngineClient = class;
  TPathInitializationEvent = procedure ( Sender : TObject; var Path : AnsiString ) of Object;
  ...
  public
    ...
    function   Run_CommandAsString(const command : AnsiString; mode : Integer) : AnsiString;
    ...
    procedure  ExecString(const command : AnsiString); {$IFDEF DELPHI4_OR_HIGHER}overload;{$ENDIF}
    procedure  ExecStrings( strings : TStrings ); {$IFDEF DELPHI4_OR_HIGHER}overload;{$ENDIF}
    function   EvalString(const command : AnsiString) : PPyObject; {$IFDEF DELPHI4_OR_HIGHER}overload;{$ENDIF}
    function   EvalStringAsStr(const command : AnsiString) : AnsiString;
...
{$ENDIF}
    function   EvalStringsAsStr( strings : TStrings ) : AnsiString;
...
function  TDynamicDll.GetQuitMessage : AnsiString;
begin
...
procedure TDynamicDll.SetDllName(const Value: AnsiString);
begin
  FDllName := Value;
end;
...
function  TPythonInterface.GetQuitMessage : AnsiString;
begin
...
function  TPythonInterface.GetUnicodeTypeSuffix : AnsiString;
begin
...
begin
  ...
  if PyDict_GetItemString(modules, PansiChar(AName)) = nil then
    begin
      PyErr_SetString(PyExc_ImportError^, PansiChar(AnsiStrings.Format('Loaded module %.200s not found in sys.modules', [AName])));
      Result := nil;
      Exit;
    end;
  Py_XINCREF(m);
...
begin
  // we build a AnsiString list of the arguments, because ParamStr returns a volatile AnsiString
  // and we want to build an array of PansiChar, pointing to valid strings.

  argc := ParamCount;
  GetMem( argv, sizeof(PansiChar)*(argc+1) );
  setlength(L,argc+1);
...
function   TPythonEngine.EvalStringAsStr(const command : AnsiString) : AnsiString;
begin
...
function   TPythonEngine.Run_CommandAsString(const command : AnsiString; mode : Integer) : AnsiString;
var
  v : PPyObject;
begin
  Result := '';
...
function TPythonEngine.EvalStringsAsStr( strings : TStrings ) : AnsiString;
begin
...
procedure TPythonEngine.RaiseError;
  ...
  function Define( E : EPythonError; const sType, sValue : AnsiString ) : EPythonError;
  begin
    E.EName  := sType;
    E.EValue := sValue;
    if sValue <> '' then
  ...
  function DefineSyntaxError( E : EPySyntaxError; const sType, sValue : AnsiString; err_type, err_value : PPyObject ) : EPySyntaxError;
  var
    s_value       : AnsiString;
    s_line        : AnsiString;
    s_filename    : AnsiString;
  ...
  function GetTypeAsString( obj : PPyObject ) : AnsiString;
  begin 
...
function TPythonEngine.PyObjectAsString( obj : PPyObject ) : AnsiString;
var
  s : PPyObject;
  i : Integer;
  tmp : PansiChar;
  ...
  begin
    ...
    with FIOPythonModule as TPythonModule do
      begin
        Engine := Self;
        ModuleName := 'pyio';
        AddMethod( 'write', pyio_write, 'write(AnsiString) -> None' );
        AddMethod( 'read',  pyio_read,  'read() -> AnsiString' );
...
function TPythonEngine.PyObjectAsVariant( obj : PPyObject ) : Variant;
  ...
  function ExtractDate( var date : Variant ) : Boolean;
    ...
    function GetStructMember( obj : PPyObject; const AMember : AnsiString ) : Word;
    var
      member : PPyObject;
    begin
...
// You must give each entry as a couple key(AnsiString)/value
function TPythonEngine.ArrayToPyDict( items : array of const) : PPyObject;
  ...
  begin
    case v.VType of
      ...
      begin
        if Assigned(v.VAnsiString) then
          Result := StrPas(PansiChar(v.VAnsiString))
        else
          Result := '';
      end;
      vtVariant:
...
procedure TPythonEngine.ListToSet( List : PPyObject; data : Pointer; size : Integer );
  ...
  begin
    ...
    tmp := PansiChar(data);
    tmp := tmp + (idx div 8);
    tmp^ := AnsiChar((Ord(tmp^) or (1 shl (idx mod 8))));
  end;
...
function TPythonEngine.GetVersion: AnsiString;
begin
  Result := '3.32';
end;

procedure TPythonEngine.SetVersion(const Value: AnsiString);
begin
  // do nothing
end;
