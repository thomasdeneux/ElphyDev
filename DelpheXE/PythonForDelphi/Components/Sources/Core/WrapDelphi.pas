...
class TTestForm(Form):
 ...
 History:
  ...
  - Fixed bug in TStrings.SqItem that returned a AnsiString instead of a wrapped TObject.
  ...
  - TStrings can be accessed with an integer to get the AnsiString item or with a key AnsiString to get
  ...
  - Made a breaking change when dealing with property sets:
    now we expect a sequence of strings. Each AnsiString should have the name as the enumeration in the set.
    ...
    a AnsiString like "[akLeft, akTop]".
  - Made a breaking change when dealing with property enumerations:
    return a AnsiString representing its value instead of the ordinal value.
...
Type
  ...
  protected
    ...
    class function GetTypeName : AnsiString; virtual;
    // Exposed Methods
  ...
  public
    ...
    function  GetCallable(AComponent : TObject; const APropName : AnsiString) : PPyObject; overload;
  ...
  { Subclass TRegisteredUnit to register your wrappers for a specific unit.
    See WrapDelphiForms which will wrapp some of the classes of the Forms.pas unit.
      ...
      function TFormsRegistration.Name: AnsiString;
      begin
        Result := 'Forms';
      end;
...
{$ENDIF}
  ...
  public
    ...
    procedure DefineVar(const AName : AnsiString; const AValue : Variant); overload;
    procedure DefineVar(const AName : AnsiString; AValue : TObject); overload;
    procedure RegisterDelphiWrapper(AWrapperClass : TPyDelphiObjectClass);
    ...
    function  GetHelperType(TypeName : AnsiString) : TPythonType;
...
{$ENDIF}
  ...
  function  CheckIndex(AIndex, ACount : Integer; const AIndexName : AnsiString = 'Index') : Boolean;
  function  CheckIntAttribute(AAttribute : PPyObject; const AAttributeName : AnsiString; var AValue : Integer) : Boolean;
  function  CheckBoolAttribute(AAttribute : PPyObject; const AAttributeName : AnsiString; var AValue : Boolean) : Boolean;
  function  CheckStrAttribute(AAttribute : PPyObject; const AAttributeName : AnsiString; var AValue : AnsiString) : Boolean;
  function  CheckObjAttribute(AAttribute : PPyObject; const AAttributeName : AnsiString;
                              AExpectedClass : TClass;
                              var AValue : TObject) : Boolean;
  function  CheckCallableAttribute(AAttribute : PPyObject; const AAttributeName : AnsiString) : Boolean;
  function  CheckEnum(const AEnumName : AnsiString; AValue, AMinValue, AMaxValue : Integer) : Boolean;
...
var
  gNames : TStringList;
function AddName(const AName : AnsiString) : PansiChar;
begin
  gNames.Add(AName);
...
begin
  if (AIndex < 0) or (AIndex >= ACount) then
    ...
    begin
      Result := False;
      PyErr_SetString (PyExc_IndexError^, PansiChar(Format('%s "%d" out of range',[AIndexName, AIndex])));
    end
  else
    Result := True;
end;
...
begin
  ...
  begin
    ...
    with GetPythonEngine do
      PyErr_SetString (PyExc_AttributeError^,
        PansiChar(Format('%s receives only AnsiString values', [AAttributeName])));
  end;
end;
...
asm
        ...
        {       ECX Pointer to result AnsiString    }
...
asm
        { ->    EAX Pointer to instance         }
        {       EDX Pointer to property info    }
        {       ECX Pointer to AnsiString value     }
...
var
  // This is the value returned when a NULL is converted into a AnsiString.  Other
  //  environments return 'NULL' instead of Delphi's default of an empty AnsiString.
...
class function TContainerAccess.Name: AnsiString;
begin
...
class function TPyDelphiObject.GetTypeName : AnsiString;
begin
...
function TEventHandlers.GetCallable(AComponent: TObject;
  const APropName: AnsiString): PPyObject;
var
  _propInfo : PPropInfo;
begin
...
procedure TPyDelphiWrapper.DefineVar(const AName: AnsiString; const AValue: Variant);
var
  _obj : PPyObject;
begin
  CheckEngine;
...
procedure TPyDelphiWrapper.DefineVar(const AName: AnsiString; AValue: TObject);
var
  _obj : PPyObject;
begin
  Assert(Assigned(Module));
...
function TPyDelphiWrapper.GetHelperType(TypeName: AnsiString): TPythonType;
var
  Index : integer;
begin
