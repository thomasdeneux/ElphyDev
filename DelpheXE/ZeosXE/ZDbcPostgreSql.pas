...
TZPgCharactersetType = (
  ...
  protected
    function GetPlainDriver(const Url: AnsiString): IZPostgreSQLPlainDriver;
  public
    constructor Create;
    function Connect(const Url: AnsiString; Info: TStrings): IZConnection; override;
  ...
  IZPostgreSQLConnection = interface(IZConnection)
    ...
    function GetTypeNameByOid(Id: Oid): AnsiString;
  ...
  public
    constructor Create(Driver: IZDriver; const Url: AnsiString;
      PlainDriver: IZPostgreSQLPlainDriver; const HostName: AnsiString; Port: Integer;
      const Database: AnsiString; const User: AnsiString; const Password: AnsiString; Info: TStrings);
    ...
    function CreateSequence(const Sequence: AnsiString; BlockSize: Integer): IZSequence; override;

    procedure Commit; override;
    procedure Rollback; override;
    //2Phase Commit Support initially for PostgresSQL (firmos) 21022006
    procedure PrepareTransaction(const transactionid: AnsiString);override;
    procedure CommitPrepared(const transactionid:AnsiString);override;
    procedure RollbackPrepared(const transactionid:AnsiString);override;
  ...
  public
    ...
    function  GetCurrentValueSQL:AnsiString;override;
    function  GetNextValueSQL:AnsiString;override;
  end;
...
function TZPostgreSQLDriver.Connect(const Url: AnsiString; Info: TStrings): IZConnection;
var
  TempInfo: TStrings;
  HostName, Database, UserName, Password: AnsiString;
  Port: Integer;
...
function TZPostgreSQLDriver.GetPlainDriver(const Url: AnsiString): IZPostgreSQLPlainDriver;
var
  Protocol: AnsiString;
begin
...
constructor TZPostgreSQLConnection.Create(Driver: IZDriver; const Url: AnsiString;
  PlainDriver: IZPostgreSQLPlainDriver; const HostName: AnsiString; Port: Integer;
  const Database, User, Password: AnsiString; Info: TStrings);
begin
...
{**
  Builds a connection AnsiString for PostgreSQL.
  @return a built connection AnsiString.
}
...
procedure TZPostgreSQLConnection.PrepareTransaction(const transactionid: AnsiString);
...
function TZPostgreSQLConnection.CreatePreparedStatement(
  const SQL: AnsiString; Info: TStrings): IZPreparedStatement;
begin
  if IsClosed then Open;
...
procedure TZPostgreSQLConnection.CommitPrepared(const transactionid: AnsiString);
var  QueryHandle: PZPostgreSQLResult;
     SQL: PAnsiChar;
     Temp: AnsiString;
begin
...
procedure TZPostgreSQLConnection.RollbackPrepared(const transactionid: AnsiString);
var  QueryHandle: PZPostgreSQLResult;
     SQL: PAnsiChar;
     Temp: AnsiString;
begin
...
{**
  ...
  @return a type name or empty AnsiString if there was no such type found.
}
function TZPostgreSQLConnection.GetTypeNameByOid(Id: Oid): AnsiString;
var
  I, Index: Integer;
  QueryHandle: PZPostgreSQLResult;
  SQL: PAnsiChar;
...
function TZPostgreSQLConnection.CreateSequence(const Sequence: AnsiString;
...
function TZPostgreSQLSequence.GetCurrentValueSQL: AnsiString;
begin
...
function TZPostgreSQLSequence.GetNextValueSQL: AnsiString;
begin
