...
type
  ...
  IZCachedResolver = interface (IZInterface)
    ...
    procedure PostUpdatesGS(Sender: IZCachedResultSet; UpdateType: TZRowUpdateType;
      OldRowAccessor, NewRowAccessor: TZRowAccessor; keys:AnsiString);
  ...
  protected
    ...
    procedure PostRowUpdatesGS(OldRowAccessor,
      NewRowAccessor: TZRowAccessor; keys:AnsiString); virtual;
    ...
    {END PATCH [1214009] CalcDefaults in TZUpdateSQL and Added Methods to GET the DB NativeResolver}
  public
    constructor CreateWithStatement(SQL: AnsiString; Statement: IZStatement);
    constructor CreateWithColumns(ColumnsInfo: TObjectList; SQL: AnsiString);
    ...
    procedure UpdateRowGS(keys:AnsiString); override;
    ...
    procedure PostUpdatesGS(keys:AnsiString); virtual;
  ...
  public
    constructor Create(ResultSet: IZResultSet; SQL: AnsiString;
...
constructor TZAbstractCachedResultSet.CreateWithStatement(SQL: AnsiString;
...
constructor TZAbstractCachedResultSet.CreateWithColumns(
  ColumnsInfo: TObjectList; SQL: AnsiString);
begin
...
procedure TZAbstractCachedResultSet.PostRowUpdatesGS(OldRowAccessor,
  NewRowAccessor: TZRowAccessor; keys:AnsiString);
begin
{$IFNDEF DISABLE_CHECKING}
  if Resolver = nil then
    raise EZSQLException.Create(SResolverIsNotSpecified);
{$ENDIF}
  Resolver.PostUpdatesGS(Self, NewRowAccessor.RowBuffer.UpdateType,
    OldRowAccessor, NewRowAccessor,keys);
end;
...
procedure TZAbstractCachedResultSet.PostUpdatesGS(keys:AnsiString);
begin
  CheckClosed;
  if FInitialRowsList.Count > 0 then
  begin
    ...
    begin
      ...
      { Posts row updates and processes the exceptions. }
      PostRowUpdatesGS(OldRowAccessor, NewRowAccessor, keys);
...
procedure TZAbstractCachedResultSet.UpdateRowGS(keys:AnsiString);
begin
  CheckUpdatable;
  if (RowNo < 1) or (RowNo > LastRowNo) then
    raise EZSQLException.Create(SCanNotUpdateEmptyRow);
...
constructor TZCachedResultSet.Create(ResultSet: IZResultSet; SQL: AnsiString;
  Resolver: IZCachedResolver);
begin
