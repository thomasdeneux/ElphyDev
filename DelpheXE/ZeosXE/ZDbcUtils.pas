...
function ResolveConnectionProtocol(Url: AnsiString;
  SupportedProtocols: TStringDynArray): AnsiString;
...
procedure ResolveDatabaseUrl(const Url: AnsiString; Info: TStrings;
  var HostName: AnsiString; var Port: Integer; var Database: AnsiString;
  var UserName: AnsiString; var Password: AnsiString; ResultInfo: TStrings);
...
function DefineColumnTypeName(ColumnType: TZSQLType): AnsiString;
...
function DefineStatementParameter(Statement: IZStatement; const ParamName: AnsiString;
  const Default: AnsiString): AnsiString;

{**
  AnsiQuotedStr or NullText
  @param S the AnsiString
  @param NullText the "NULL"-Text
  @param QuoteChar the AnsiChar that is used for quotation
  @return 'null' if S is '', otherwise AnsiQuotedStr(S)
}
function AQSNullText(const Value, NullText: AnsiString; QuoteChar: AnsiChar = ''''): AnsiString;
...
function AQSNull(const Value: AnsiString; QuoteChar: AnsiChar = ''''): AnsiString;

{**
  ToLikeString returns the given AnsiString or if the AnsiString is empty it returns '%'
  @param Value the AnsiString
  @return given Value or '%'
}
function ToLikeString(const Value: AnsiString): AnsiString;
...
function AnsiQuotedAnsiStr(const S: Ansistring; Quote: AnsiChar): Ansistring;
var
  i:integer;
begin
  result:=Quote;
  ...
  begin
    result:=result+quote;
    if s[i]=quote then result:=result+quote;
  end;
  result:=result+quote;
end;
{**
...
begin
  if Value = '' then
    ...
    Result := AnsiQuotedAnsiStr(Value, QuoteChar);
end;
