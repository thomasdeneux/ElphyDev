...
type
  ...
  public
    ...
    function Connect(const Url: AnsiString; Info: TStrings): IZConnection; virtual;
    function AcceptsURL(const Url: AnsiString): Boolean; virtual;

    function GetPropertyInfo(const Url: AnsiString; Info: TStrings): TStrings; virtual;
    ...
    function GetClientVersion(const Url: AnsiString): Integer; virtual;
  end;
  ...
  protected
    ...
    function CreateRegularStatement(Info: TStrings): IZStatement;
      virtual;
    function CreatePreparedStatement(const SQL: AnsiString; Info: TStrings):
      IZPreparedStatement; virtual;
    function CreateCallableStatement(const SQL: AnsiString; Info: TStrings):
  ...
  public
    constructor Create(Driver: IZDriver; const Url: AnsiString; const HostName: AnsiString;
      Port: Integer; const Database: AnsiString; const User: AnsiString; const Password: AnsiString;
    ...
    function CreateNotification(const Event: AnsiString): IZNotification; virtual;
    function CreateSequence(const Sequence: AnsiString; BlockSize: Integer):
      IZSequence; virtual;

    function NativeSQL(const SQL: AnsiString): AnsiString; virtual;
    ...
    procedure PrepareTransaction(const transactionid: AnsiString);virtual;
    procedure CommitPrepared(const transactionid: AnsiString);virtual;
    procedure RollbackPrepared(const transactionid: AnsiString);virtual;
    ...
    procedure SetCatalog(const Catalog: AnsiString); virtual;
    function GetCatalog: AnsiString; virtual;
  ...
  protected
    property EventName: AnsiString read FEventName write FEventName;
    property Connection: IZConnection read FConnection write FConnection;
  public
    constructor Create(Connection: IZConnection; EventName: AnsiString);
    function GetEvent: AnsiString;
    procedure Listen; virtual;
    procedure Unlisten; virtual;
    procedure DoNotify; virtual;
    function CheckEvents: AnsiString; virtual;
  ...
  protected
    ...
    property Connection: IZConnection read FConnection write FConnection;
  public
    constructor Create(Connection: IZConnection; Name: AnsiString;
      BlockSize: Integer);

    function GetCurrentValue: Int64; virtual;
    function GetNextValue: Int64; virtual;

    function GetCurrentValueSQL: AnsiString; virtual; abstract;
    function GetNextValueSQL: AnsiString; virtual; abstract;
...
{**
  ...
  AnsiString tag/value pairs as connection arguments.
  Normally at least "user" and "password" properties should be
  included in the Properties.

  @param url the URL of the database to which to connect
  @param info a list of arbitrary AnsiString tag/value pairs as
    connection arguments. Normally at least a "user" and
    "password" property should be included.
  @return a <code>Connection</code> object that represents a
    connection to the URL
}
function TZAbstractDriver.Connect(const Url: AnsiString; Info: TStrings): IZConnection;
begin
  Result := nil;
end;
...
function TZAbstractDriver.AcceptsURL(const Url: AnsiString): Boolean;
var
  I: Integer;
...
function TZAbstractDriver.GetPropertyInfo(const Url: AnsiString; Info: TStrings): TStrings;
begin
  Result := nil;
end;
...
function TZAbstractDriver.GetClientVersion(const Url: AnsiString): Integer;
begin
  Result := 0;
end;
...
{**
  ...
  @param Info a AnsiString list with extra connection parameters.
}
constructor TZAbstractConnection.Create(Driver: IZDriver; const Url: AnsiString;
  const HostName: AnsiString; Port: Integer; const Database: AnsiString; const User: AnsiString;
  const Password: AnsiString; Info: TStrings; Metadata: TContainedObject);
begin
  FDriver := Driver;
  FHostName := HostName;
  FPort := Port;
...
{**
  ...
  @param SQL a SQL query AnsiString.
  @param Info a statement parameters.
...
function TZAbstractConnection.PrepareStatement(const SQL: AnsiString): IZPreparedStatement;
begin
...
function TZAbstractConnection.PrepareStatementWithParams(const SQL: AnsiString;
  Info: TStrings): IZPreparedStatement;
begin
  Result := CreatePreparedStatement(SQL, Info);
end;

procedure TZAbstractConnection.PrepareTransaction(const transactionid: AnsiString);
begin
  RaiseUnsupportedException;
end;
...
function TZAbstractConnection.CreatePreparedStatement(const SQL: AnsiString;
  Info: TStrings): IZPreparedStatement;
begin
  Result := nil;
  RaiseUnsupportedException;
...
{**
  ...
  @param sql a SQL statement that may contain one or more '?'
    ...
    function call escape AnsiString.
  @return a new CallableStatement object containing the
    pre-compiled SQL statement
}

function TZAbstractConnection.PrepareCall(
  const SQL: AnsiString): IZCallableStatement;
begin
...
function TZAbstractConnection.PrepareCallWithParams(const SQL: AnsiString;
...
function TZAbstractConnection.CreateCallableStatement(const SQL: AnsiString;
  Info: TStrings): IZCallableStatement;
begin
  Result := nil;
  RaiseUnsupportedException;
...
function TZAbstractConnection.CreateNotification(const Event: AnsiString): IZNotification;
begin
  Result := nil;
  RaiseUnsupportedException;
end;
...
function TZAbstractConnection.CreateSequence(const Sequence: AnsiString;
...
function TZAbstractConnection.NativeSQL(const SQL: AnsiString): AnsiString;
begin
  Result := SQL;
end;
...
procedure TZAbstractConnection.CommitPrepared(const transactionid: AnsiString);
begin
  RaiseUnsupportedException;
end;
...
procedure TZAbstractConnection.RollbackPrepared(const transactionid: AnsiString);
begin
  RaiseUnsupportedException;
end;
...
{**
  Escape a AnsiString so it's acceptable for the Connection's server.
  @param value AnsiString that should be escaped
...
procedure TZAbstractConnection.SetCatalog(const Catalog: AnsiString);
begin
end;
...
function TZAbstractConnection.GetCatalog: AnsiString;
begin
  Result := '';
end;
...
function TZAbstractNotification.GetEvent: AnsiString;
begin
  Result := FEventName;
end;
...
{**
  ...
  @return a AnsiString with incoming events??
}
function TZAbstractNotification.CheckEvents: AnsiString;
begin
  Result := '';
end;

{**
  Sends a notification AnsiString.
}
...
constructor TZAbstractSequence.Create(Connection: IZConnection;
  Name: AnsiString; BlockSize: Integer);
begin
  FConnection := Connection;
  FName := Name;
...
function TZAbstractSequence.GetName: AnsiString;
begin
  Result := FName;
end;
...
{
function TZAbstractSequence.GetCurrentValueSQL: AnsiString;
begin
 result:='IMPLEMENT';
end;
}
...
{
function TZAbstractSequence.GetNextValueSQL: AnsiString;
begin
 result:='IMPLEMENT';
end;
}
...
procedure TZAbstractSequence.SetName(const Value: AnsiString);
begin
  FName := Value;
end;
