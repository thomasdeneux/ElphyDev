...
type
  ...
  public
    ...
    function ExecuteQuery(const SQL: AnsiString): IZResultSet; virtual;
    function ExecuteUpdate(const SQL: AnsiString): Integer; virtual;
    ...
    function Execute(const SQL: AnsiString): Boolean; virtual;
    ...
    procedure AddBatch(const SQL: AnsiString); virtual;
  ...
  public
    constructor Create(Connection: IZConnection; const SQL: AnsiString; Info: TStrings);
    destructor Destroy; override;

    function ExecuteQueryPrepared: IZResultSet; virtual;
    function ExecuteUpdatePrepared: Integer; virtual;
    function ExecuteUpdatePreparedGS(keys:AnsiString): Integer; virtual;
  ...
  public
    constructor Create(Connection: IZConnection; SQL: AnsiString; Info: TStrings);
  ...
  protected
    ...
    function PrepareSQLParam(ParamIndex: Integer): AnsiString; virtual; abstract;
    function GetExecStatement: IZStatement;
    function TokenizeSQLQuery: TStrings;
    function PrepareSQLQuery(keys:AnsiString): AnsiString; virtual;
  ...
  public
    ...
    function ExecuteQuery(const SQL: AnsiString): IZResultSet; override;
    function ExecuteUpdate(const SQL: AnsiString): Integer; override;
    function Execute(const SQL: AnsiString): Boolean; override;

    function ExecuteQueryPrepared: IZResultSet; override;
    function ExecuteUpdatePrepared: Integer; override;
    function ExecuteUpdatePreparedGS(keys:AnsiString): Integer; override;
...
function TZAbstractStatement.ExecuteQuery(const SQL: AnsiString): IZResultSet;
begin
  Result := nil;
...
function TZAbstractStatement.ExecuteUpdate(const SQL: AnsiString): Integer;
begin
  Result := 0;
  RaiseUnsupportedException;
end;
...
{**
  ...
  unknown SQL AnsiString.  The  methods <code>execute</code>,
...
function TZAbstractStatement.Execute(const SQL: AnsiString): Boolean;
begin
  Result := False;
  LastResultSet := nil;
  LastUpdateCount := -1;
...
procedure TZAbstractStatement.AddBatch(const SQL: AnsiString);
begin
...
constructor TZAbstractPreparedStatement.Create(Connection: IZConnection;
  const SQL: AnsiString; Info: TStrings);
begin
...
function TZAbstractPreparedStatement.ExecuteUpdatePreparedGS(keys:AnsiString): Integer;
begin
  Result := -1;
  RaiseUnsupportedException;
end;
...
{**
  ...
  or <code>LONGVARCHAR</code> parameter as a <code>AnsiString</code> in
  the Java programming language.
  <p>
  For the fixed-length type JDBC <code>CHAR</code>,
  the <code>AnsiString</code> object
...
(*
function TZEmulatedPreparedStatement.PrepareSQLQuery: AnsiString;
var
  I: Integer;
  ParamIndex: Integer;
  Tokens: TStrings;
...
function TZEmulatedPreparedStatement.PrepareSQLQuery(keys:AnsiString): AnsiString;
var
  I: Integer;
  ParamIndex: Integer;
  Tokens: TStrings;
...
function getIdent(st:AnsiString):AnsiString;
const
...
begin
  ...
  if k2>0 then
  begin
    while (k2>0) and (st[k2]=' ') do dec(k2);
    k1:=k2-1;

    while (k1>0) and (st[k1] in letters) do dec(k1);
    result:=copy(st,k1+1,k2-k1-1);

    if (length(result)>1) and (result[1]='"') and (result[length(result)]='"')
      then result:=copy(result,2,length(result)-2);
  end;
end;

procedure Process1(st:AnsiString);
begin
  st:=getIdent(st);
  if st<>'' then list.Add(st);
end;

function test1(st:AnsiString): boolean;  // true if must be removed
begin
  st:=getIdent(st);
  result:= (st<>'') and
           (
...
procedure ProcessWhere;
var
  k:integer;
begin
  if IsWhere then exit;

  k:=pos('WHERE',tok);
  IsWhere:= (k>0);
  if (k>1) and (tok[k-1]<>' ') then IsWhere:=false;
  if (k+5<length(tok)) and (tok[k+5]<>' ') then IsWhere:=false;

  if IsWhere then
  begin
    result:=result+copy(tok,1,k+4);
    delete(tok,1,k+4);
    firstTok:=true;
  end;
end;
...
begin
  ...
  begin
    ...
    if not IsWhere then process1(Tok)
    else
    if flag=0 then Flag:= ord(Test1(Tok))*2;

    if flag=0 then
    begin
      if Tok ='?' then Result := Result + PrepareSQLParam(ParamIndex)
      else
      begin
        if FirstTok then
        begin
          st1:=tok;
          while (length(st1)>0) and (st1[1]=' ') do delete(st1,1,1);
          if copy(st1,1,4)='AND ' then tok:=' '+copy(st1,5,1000);
          FirstTok:=false;
        end;
        Result := Result + Tok;
      end;
...
function TZEmulatedPreparedStatement.Execute(const SQL: AnsiString): Boolean;
begin
...
function TZEmulatedPreparedStatement.ExecuteQuery(const SQL: AnsiString): IZResultSet;
begin
...
function TZEmulatedPreparedStatement.ExecuteUpdate(const SQL: AnsiString): Integer;
begin
...
function TZEmulatedPreparedStatement.ExecuteUpdatePreparedGS(keys:AnsiString): Integer;
begin
  Result := ExecuteUpdate(PrepareSQLQuery(keys));
end;
