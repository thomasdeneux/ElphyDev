...
{Version modifiée pour tenir compte des systèmes à plusieurs moniteurs

 DirectDrawEnumerate est simplement remplacé par DirectDrawEnumerateEx

 Le nombre de backbuffers est fixé à 3 pour TDXDrawDriverFlip
}
...
uses
  ...
  DXClassG, DIBG, DXTexImgG, DirectXG,
  util1,debug0;
...
{$ENDIF}
  ...
  private
    ...
    function SetSize(AWidth, AHeight, ABitCount,AFreq: Integer): Boolean;
    function DynSetSize(AWidth, AHeight, ABitCount,AFreq: Integer): Boolean;
    procedure SetRefreshRate(value:Dword);
  public
    constructor Create(ADXDraw: TCustomDXDraw);
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    function IndexOf(Width, Height, BitCount,freq: Integer): Integer;
  ...
  published
    ...
    property RefreshRate: Dword read FrefreshRate write SetRefreshRate;
  end;
  ...
  public
    property canvas;         { J'ajoute la propriété de TcustomControl
                               C'est le canvas ordinaire. Pas celui de surface !
                             }
  end;
...
function stGUID(g:PGUID):AnsiString;
type
...
begin
  st:='';
  if assigned(g) then
    for i:=1 to 16 do st:=st+Istr(P16(g)^[i])+' ';
  result:=st;
end;


type
  TDirectDrawEnumerateEx = function(lpCallback: TDDEnumCallbackA;lpContext: Pointer; dwFlags:DWORD): HRESULT; stdcall;

function DXDirectDrawEnumerateEx(lpCallback: TDDEnumCallbackA; lpContext: Pointer): HRESULT;
begin
  Result := TDirectDrawEnumerateEx(DXLoadLibrary('DDraw.dll', 'DirectDrawEnumerateExA'))
    (lpCallback, lpContext,7);
end;
...
var
  ...
  function DDENUMCALLBACK(lpGuid: PGUID; lpstrDescription: LPCSTR;
    lpstrModule: LPCSTR; lpContext: Pointer;monitor:Hmonitor): BOOL; stdcall;
  begin
    Result := True;
    with TDirectXDriver.Create(TDirectXDrivers(lpContext)) do
    begin
      ...
      Description := lpstrDescription;
      DriverName := lpstrModule;
      //messageCentral(Description+CRLF+DriverName+CRLF+stGuid(guid)+crlf+Istr(monitor));
    end;
  end;
...
begin
  ...
  begin
    DirectDrawDrivers := TDirectXDrivers.Create;
    try
      DXDirectDrawEnumerateEx(@DDENUMCALLBACK, DirectDrawDrivers);
    except
      DirectDrawDrivers.Free;
      raise;
    end;
...
{$IFDEF FPC}
function TDirectDrawSurfaceCanvas.getWidth: integer;
begin
  if (FSurface.IDDSurface<>nil)
    then result:=Fsurface.Width
    else result:=0;
end;

function TDirectDrawSurfaceCanvas.getHeight: integer;
begin
  if (FSurface.IDDSurface<>nil)
    then result:=Fsurface.Height
    else result:=0;
end;
{$ENDIF}
    ...
    begin
      try
        case ddsd.ddpfPixelFormat.dwRGBBitCount of
          1 : Result := intG(PByte(intG(ddsd.lpSurface)+
                Y*ddsd.lPitch+(X shr 3))^ and (1 shl (X and 7))<>0);
          4 : begin
                if X and 1=0 then
                  Result := PByte(intG(ddsd.lpSurface)+Y*ddsd.lPitch+(X shr 1))^ shr 4
                else
                  Result := PByte(intG(ddsd.lpSurface)+Y*ddsd.lPitch+(X shr 1))^ and $0F;
              end;
          8 : Result := PByte(intG(ddsd.lpSurface)+Y*ddsd.lPitch+X)^;
          16: Result := PWord(intG(ddsd.lpSurface)+Y*ddsd.lPitch+X*2)^;
          24: with PRGB(intG(ddsd.lpSurface)+Y*ddsd.lPitch+X*3)^ do
                Result := R or (G shl 8) or (B shl 16);
          32: Result := PInteger(intG(ddsd.lpSurface)+Y*ddsd.lPitch+X*4)^;
        end;
      finally
        UnLock;
      end;
...
function TDirectDrawSurface.GetScanLine(Y: Integer): pointer;
begin
  Result := nil;
  if (IDDSurface<>nil) and (Y>=0) and (Y<Height) and (FlockCount>0) then
    with FLockSurfaceDesc do
    Result := Pointer(intG(lpSurface)+ Y*lPitch);
end;
  ...
  if (IDDSurface<>nil) and (X>=0) and (X<Width) and (Y>=0) and (Y<Height) then
    ...
    begin
      try
        case ddsd.ddpfPixelFormat.dwRGBBitCount of
          1 : begin
                P := PByte(intG(ddsd.lpSurface)+Y*ddsd.lPitch+(X shr 3));
                if Value=0 then
          ...
          4 : begin
                P := PByte(intG(ddsd.lpSurface)+Y*ddsd.lPitch+(X shr 1));
                if X and 1=0 then
                  P^ := (P^ and $0F) or (Value shl 4)
                else
                  P^ := (P^ and $F0) or (Value and $0F);
              end;
          8 : PByte(intG(ddsd.lpSurface)+Y*ddsd.lPitch+X)^ := Value;
          16: PWord(intG(ddsd.lpSurface)+Y*ddsd.lPitch+X*2)^ := Value;
          24: with PRGB(intG(ddsd.lpSurface)+Y*ddsd.lPitch+X*3)^ do
              begin
                R := Byte(Value);
                G := Byte(Value shr 8);
          ...
          32: PInteger(intG(ddsd.lpSurface)+Y*ddsd.lPitch+X*4)^ := Value;
        end;
      finally
        UnLock;
      end;
...
function TDXDrawDisplayMode.GetRefreshRate: Integer;
begin
  Result := FSurfaceDesc.dwRefreshRate;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FrefreshRate := TDXDrawDisplay(Source).RefreshRate;
...
begin
  ...
  begin
    ...
    with ddsd do
      i := IndexOf(dwWidth, dwHeight, ddpfPixelFormat.dwRGBBitCount,dwRefreshRate);
    if i<>-1 then
      Result := Modes[i];
  end;
  if Result=nil then
...
function TDXDrawDisplay.IndexOf(Width, Height, BitCount,freq: Integer): Integer;
var
  i: Integer;
begin
  Result := -1;
  for i:=0 to Count-1 do
    if (Modes[i].Width=Width) and (Modes[i].Height=Height) and
       (Modes[i].BitCount=BitCount) and (Modes[i].RefreshRate=Freq)  then
    begin
      Result := i;
      Exit;
    end;
...
begin
  ...
  if Item1.Height<>Item2.Height then
    Result := Item1.Height-Item2.Height
  else
  if Item1.BitCount<>Item2.BitCount then
    Result := Item1.BitCount-Item2.BitCount
  else
    Result:=Item1.RefreshRate-item2.RefreshRate;
end;
...
begin
  ...
  if FDXDraw.DDraw<>nil then
  begin
    FDXDraw.DDraw.DXResult := FDXDraw.DDraw.IDraw.EnumDisplayModes(DDEDM_REFRESHRATES, PDDSurfaceDesc(nil)^,
      FModes, @EnumDisplayModesProc);
    if FrefreshRate=0
      then FDXDraw.Ddraw.IDraw.GetMonitorFrequency(FRefreshRate);
    {statusLineTxt(Istr(FRefreshRate));}
  end
  else
  begin
    DDraw := TDirectDraw.Create(PGUID(FDXDraw.FDriver));
    try
      DDraw.IDraw.EnumDisplayModes(DDEDM_REFRESHRATES, PDDSurfaceDesc(nil)^, FModes,
                                   @EnumDisplayModesProc);
      if FrefreshRate=0
        then Ddraw.IDraw.GetMonitorFrequency(FRefreshRate);
    finally
      DDraw.Free;
    end;
  end;
...
function TDXDrawDisplay.SetSize(AWidth, AHeight, ABitCount,Afreq: Integer): Boolean;
begin
  Result := False;
  if FDXDraw.DDraw<>nil then
  begin
    if Afreq=0 then
      FDXDraw.DDraw.DXResult :=
        FDXDraw.DDraw.IDraw.SetDisplayMode(AWidth, AHeight, ABitCount)
    else
      FDXDraw.DDraw.DXResult :=
        FDXDraw.DDraw.IDraw7.SetDisplayMode(AWidth, AHeight, ABitCount,Afreq,0);
    ...
    begin
      ...
      FHeight := AHeight;
      FBitCount := ABitCount;
      if Afreq<>0 then FrefreshRate:=Afreq;
    end;

    {messageCentral('SetDisplayMode '+Bstr(result));}
  end;
end;

function TDXDrawDisplay.DynSetSize(AWidth, AHeight, ABitCount,Afreq: Integer): Boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        if IndexOf(DWidth, DHEight, ABitCount,Afreq)<>-1 then
          DBitCount := ABitCount;
      end;

      Result := SetSize(DWidth, DHeight, DBitCount,Afreq);
    end;
  end;
...
begin
  ...
  if SetSize(AWidth, AHeight, ABitCount,Afreq) then
  begin
    Result := True;
    Exit;
  end;
...
procedure TDXDrawDisplay.SetRefreshRate(Value: Dword);
begin
  FrefreshRate := Value;
end;
...
begin
  ...
  begin
    ...
    DXDirectDrawEnumerateEx(@EnumDirectDrawDriverCallback, @rec)
  end else
  begin
...
type
  ...
  private
    ...
    procedure Flip(wait:boolean); virtual; abstract;
...
procedure TDXDrawDriverBlt.Flip(wait:boolean);
var
  pt: TPoint;
  Dest: TRect;
  DF: TDDBltFX;
...
procedure TDXDrawDriverFlip.Flip(wait:boolean);
var
  flags:integer;
begin
  if wait
    then flags:=DDFLIP_WAIT
    else flags:=0;

  if (FDXDraw.FForm<>nil) and (FDXDraw.FForm.Active) OR FDXDraw.SecondScreen then
    FDXDraw.FPrimary.DXResult := FDXDraw.FPrimary.ISurface.Flip(nil, flags)
  else
...
{  messageCentral('dwBackBufferCount='+Istr(PrimaryDesc.dwBackBufferCount));}
...
procedure TCustomDXDraw.FormWndProc(var Message: TMessage; DefWindowProc: TWndMethod);
  ...
  begin
    if Initialized and (FNowOptions*[doFullScreen, doFlip]=[doFullScreen, doFlip])

      then DDraw.IDraw.FlipToGDISurface;
  end;
  ...
  case Message.Msg of
    ...
    WM_WINDOWPOSCHANGED:
        begin
          ...
          {$IFDEF Win64}
          if assigned(PwindowPos( message.Lparam)) and (PwindowPos( message.Lparam)^.flags and SWP_SHOWWINDOW<>0) then
          {$ELSE}
          if assigned( TWMWindowPosChanged(Message).WindowPos) and (TWMWindowPosChanged(Message).WindowPos^.flags and SWP_SHOWWINDOW<>0) then
          {$ENDIF}
          begin
            DefWindowProc(Message);
            if AutoInitialize and (not FInitialized2) then
    ...
    WM_ACTIVATE:
        begin
          if not secondScreen then
          {$IFDEF Win64}
          if word(Message.wparam)=WA_INACTIVE then
          {$ELSE}
    ...
    WM_INITMENU:
        begin
          if not secondScreen then FlipToGDISurface;
        end;
    WM_DESTROY:
        begin
          Finalize;
...
procedure TCustomDXDraw.Flip(wait:boolean);
begin
  if Initialized and (not FUpdating) then
  begin
    if TryRestore then
      TDXDrawDriver(FDXDrawDriver).Flip(wait);
  end;
end;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        if not Display.DynSetSize(Display.Width, Display.Height, Display.BitCount,
                                  Display.FrefreshRate ) then
          raise EDXDrawError.CreateFmt(SDisplaymodeChange+crlf,
                                       [Display.Width, Display.Height, Display.BitCount]);
      end;
...
begin
  ...
  begin
    {$IFNDEF WIN64} asm FInit end; {$ENDIF}
    FViewport.Clear;
...
begin
  ...
  {messageCentral('setCoop '+Istr(DDraw.DXresult));}
end;
...
procedure TDirect3DTexture2.Initialize;
  ...
  begin
    ...
    begin
      ...
      begin
        {  Bit count check  }
        if Abs(intG(lpDDPixFmt.dwRGBBitCount)-idx_index)>
          Abs(intG(tex.FTextureFormat.ddpfPixelFormat.dwRGBBitCount)-idx_index) then Exit;
...
const
  ...
  begin
    case ddsd.ddpfPixelFormat.dwRGBBitCount of
      1 : PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x div 8)^ :=
            (PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x div 8)^ and (not Mask1[x mod 8])) or (c shl Shift1[x mod 8]);
      2 : PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x div 4)^ :=
            (PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x div 4)^ and (not Mask2[x mod 4])) or (c shl Shift2[x mod 4]);
      4 : PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x div 2)^ :=
            (PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x div 2)^ and (not Mask4[x mod 2])) or (c shl Shift4[x mod 2]);
      8 : PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x)^ := c;
      16: PWord(intG(ddsd.lpSurface)+ddsd.lPitch*y+x*2)^ := c;
      24: begin
            PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x*3)^ := c shr 0;
            PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x*3+1)^ := c shr 8;
            PByte(intG(ddsd.lpSurface)+ddsd.lPitch*y+x*3+2)^ := c shr 16;
          end;   
      32: PDWORD(intG(ddsd.lpSurface)+ddsd.lPitch*y+x*4)^ := c;
    end;
  end;
    ...
    begin
      try
        if (SrcImage.idx_index.Mask=DWORD(1 shl ddsd.ddpfPixelFormat.dwRGBBitCount)-1) and (SrcImage.idx_alpha.Mask=0) and
          (SrcImage.BitCount=intG(ddsd.ddpfPixelFormat.dwRGBBitCount)) and (not SrcImage.PackedPixelOrder) then
        begin
          for y:=0 to ddsd.dwHeight-1 do
            Move(SrcImage.ScanLine[y]^, Pointer(intG(ddsd.lpSurface)+ddsd.lPitch*y)^, (intG(ddsd.dwWidth)*SrcImage.BitCount+7) div 8);
        end else
        begin
          for y:=0 to ddsd.dwHeight-1 do
          begin
    ...
    begin
      try
        ...
        if (dest_red_fmt.Mask=SrcImage.rgb_red.Mask) and (dest_green_fmt.Mask=SrcImage.rgb_green.Mask) and
          ...
          (intG(ddsd.ddpfPixelFormat.dwRGBBitCount)=SrcImage.BitCount) and (not SrcImage.PackedPixelOrder) then
        begin                
          for y:=0 to ddsd.dwHeight-1 do
            Move(SrcImage.ScanLine[y]^, Pointer(intG(ddsd.lpSurface)+ddsd.lPitch*y)^, (intG(ddsd.dwWidth)*SrcImage.BitCount+7) div 8);
        end else
...
procedure Register;
begin
  RegisterComponents('DXG',      [TDXDraw]);
end;


Initialization
AffDebug('Initialization DXDrawsG',0);
finalization
  DirectDrawDrivers.Free;
end.
