...
{ TDIB implémente un objet identique à Tbitmap
  Sous Windows, on pourrait facilement s'en passer mais sous Lazarus, il est très utile
  car l'objet Tbitmap est très différent de celui de Delphi: il manque l'accès direct
  avec scanline.

  DibG ne supporte pas la transparence! A faire.
}
...
type
  TRGBQuads = array[0..255] of TRGBQuad;             // TRGBquad = stockage B G R f

  TPaletteEntries = array[0..255] of TPaletteEntry;  // TPaletteEntry = stockage R G B f
  ...
  private
    ...
    procedure StartProgress(const Name: AnsiString);
...
{    function SaveImage(var f:file):integer;overload;}
    function SaveImage(f: TStream):integer;overload;

    procedure initRgbDib(w,h:integer);

    function ColorFromDev(devcol: integer): integer;
    function devColor(col:integer):integer;
    procedure fillRect(rect:Trect;devcol:integer);
    procedure fill(devcol:integer);

    function extractDib8(x1,y1,x2,y2:integer):Tdib;
    function extractDib(x1,y1,x2,y2:integer):Tdib;

    procedure CopyDibRect(DestRect: Trect; SrcDib: Tdib; srcRect: Trect);
    procedure CopyRectTo(Acanvas:Tcanvas;srcRect:Trect);
...
function RawColorToRgb(const PixelFormat: TDIBPixelFormat; Color: DWORD):integer;
function RgbToRawColor(const PixelFormat: TDIBPixelFormat; Color: integer):integer;

function GreyscaleColorTable: TRGBQuads;
//function MonoColorTable: TRGBQuads;   stmdef contient aussi MonoColorTable
...
function StdColorTable: TRGBQuads;
function GreyscaleMixColorTable: TRGBQuads;

procedure LoadDibFromFile(bm:Tdib; stF:string);
function SaveDibToFile(bm:Tdib; stF:string; const quality:integer=100): boolean;
...
var
  r,g,b:byte;
begin
  pfGetRgb(PixelFormat,color,r,g,b);
  result:=rgb(r,g,b);
end;

function RgbToRawColor(const PixelFormat: TDIBPixelFormat; Color: integer):integer;
begin
  result:=pfRgb(PixelFormat,color and $FF, (color shr 8) and $FF, (color shr 16) and $FF);
end;
...
function MonoColorTable: TRGBQuads;
var
  i: Integer;
begin
  with Result[0] do
...
begin
  ...
  for j:=0 to 3 do
  for k:=0 to 3 do
    if not( (i=0) and (j=0) and (k=0) or (i=3) and (j=3) and (k=3) ) then
    with Result[r] do
    begin
      if i<3 then rgbRed := i*64 else rgbRed:=255;
      if j<3 then rgbGreen := j*64 else rgbGreen:=255;
      if k<3 then rgbBlue := k*64 else rgbBlue:=255;
      rgbReserved := 0;
      inc(r);
    end;
  ...
  for i:=r to 255 do
    with Result[i] do
    begin
      rgbRed :=  round((i-r)/(255-r)*255);
      rgbGreen := round((i-r)/(255-r)*255);
      rgbBlue := round((i-r)/(255-r)*255);
      rgbReserved := 0;
    end;
end;
...
begin
  ...
  for j:=1 to 6 do
  for k:=1 to 7 do
    with Result[i*j*k-1] do
    begin
      rgbRed := (i-1)*255 div 5;
      rgbGreen := (j-1)*255 div 5;
      rgbBlue := (k-1)*255 div 6;
      rgbReserved := 0;
    end;
...
//  Create;
  ...
  if UsePixelFormat then
  begin
    with PLocalDIBPixelFormat(intG(FBitmapInfo)+InfoOfs)^ do
    begin
  ...
  Move(FColorTable, Pointer(intG(FBitmapInfo)+FColorTablePos)^, SizeOf(TRGBQuad)*FPaletteCount);
  ...
  FTopPBits := Pointer(intG(FPBits)+(FHeight-1)*FWidthBytes);
end;
...
procedure TDIBSharedImage.Compress(Source: TDIBSharedImage);
  ...
  var
    ...
    begin
      ...
      Result := Pointer(intG(FPBits)+Size);
      Inc(Size);
    end;
  ...
  begin
    ...
    begin
      x := 0;
      Src := Pointer(intG(Source.FPBits)+y*FWidthBytes);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            PByte(intG(FPBits)+PB1)^ := B1;
            PByte(intG(FPBits)+PB2)^ := B2;
          end;
        end;
  ...
  begin
    ...
    begin
      ...
      while x<Source.FWidth do
      begin
        if (Source.FWidth-x>2) and (Src^=PByte(intG(Src)+1)^) then
        begin
          {  Encoding mode  }
          B1 := 2;
          B2 := Src^;
        ...
        if (Source.FWidth-x>2) and (Src^<>PByte(intG(Src)+1)^) and (PByte(intG(Src)+1)^=PByte(intG(Src)+2)^) then
        begin
          {  Encoding mode }
          AllocByte^ := 1;
        ...
        begin
          ...
          begin
            ...
            begin
              if (Source.FWidth-x>3) and (Src^=PByte(intG(Src)+1)^) and (Src^=PByte(intG(Src)+2)^) and (Src^=PByte(intG(Src)+3)^) then
                Break;
    ...
    begin
      ...
      begin
        case B2 of
          ...
          {  Absolute mode  }
          Dest := Pointer(intG(FPBits)+Y*FWidthBytes);
    ...
    begin
      ...
      begin
        case B2 of
          0: begin  {  End of line  }
               X := 0; Inc(Y);
               Dest := Pointer(intG(FPBits)+Y*FWidthBytes+X);
             end;
          1: Break; {  End of bitmap  }
          2: begin  {  Difference of coordinates  }
  ...
  begin
    ...
    begin
      for y:=0 to Abs(BI.biHeight)-1 do
        Stream.ReadBuffer(Pointer(intG(FTopPBits)+y*FNextLine)^, FWidthBytes);
    end else
    begin
      Stream.ReadBuffer(FPBits^, FSize);
    end;
...
begin
  ...
  case BI.biSize of
    SizeOf(TBitmapCoreHeader):
      begin
        {  OS/2 type  }
        Stream.ReadBuffer(Pointer(IntG(@BC)+4)^, SizeOf(TBitmapCoreHeader)-4);
    ...
    SizeOf(TBitmapInfoHeader):
      begin
        {  Windows type  }
        Stream.ReadBuffer(Pointer(IntG(@BI)+4)^, SizeOf(TBitmapInfoHeader)-4);
      end;
  else
    raise EInvalidGraphic.Create(SInvalidDIB);
  end;
  ...
  begin
    ...
    try
      ...
      try
        ...
        begin
          with Rect do
            StretchBlt(ACanvas.Handle, Left, Top, Right - Left, Bottom - Top, FImage.FDC, 0, 0, Width, Height, ACanvas.CopyMode);
        end;
      finally
...
begin
  ...
  if not FImage.FMemoryImage then
    GDIFlush;
  Result := Pointer(intG(FTopPBits)+Y*FNextLine);
end;
...
{En mode 24 bits, R G et B sont inversés par rapport à la version de Hori}
...
begin
  ...
  begin
    case FBitCount of
      1 : Result := (PArrayByte(intG(FTopPBits)+Y*FNextLine)[X shr 3] and Mask1[X and 7]) shr Shift1[X and 7];
      4 : Result := (PArrayByte(intG(FTopPBits)+Y*FNextLine)[X shr 1] and Mask4[X and 1]) shr Shift4[X and 1];
      8 : Result := PArrayByte(intG(FTopPBits)+Y*FNextLine)[X];
      16: Result := PArrayWord(intG(FTopPBits)+Y*FNextLine)[X];
      24: with PArrayBGR(intG(FTopPBits)+Y*FNextLine)[X] do
            Result := B or (G shl 8) or (R shl 16);
      32: Result := PArrayDWord(intG(FTopPBits)+Y*FNextLine)[X];
    end;
  end;
end;
...
begin
  ...
  begin
    case FBitCount of
      1 : begin
            P := @PArrayByte(intG(FTopPBits)+Y*FNextLine)[X shr 3];
      ...
      8 : PArrayByte(intG(FTopPBits)+Y*FNextLine)[X] := Value;
      16: PArrayWord(intG(FTopPBits)+Y*FNextLine)[X] := Value;
      24: with PArrayBGR(intG(FTopPBits)+Y*FNextLine)[X] do
          begin
            R := Byte(Value shr 16);
            G := Byte(Value shr 8);
      ...
      32: PArrayDWord(intG(FTopPBits)+Y*FNextLine)[X] := Value;
    end;
  end;
end;
...
procedure TDIB.StartProgress(const Name: AnsiString);
begin
  FProgressName := Name;
  FProgressOld := 0;
...
begin
  Redraw := (GetTickCount-FProgressOldTime>200) and (FProgressY-FProgressOldY>32) and
    (((Height div 3>intG(FProgressY)) and (FProgressOldY=0)) or (FProgressOldY<>0));
...
var
  ...
  begin
    case Temp.BitCount of
      1 : begin
            SrcP := Pointer(intG(Temp.TopPBits)+Y*Temp.NextLine);
            AveP := @Ave;
            for x:=0 to XCount-1 do
            begin
              with Temp.ColorTable[(PByte(intG(SrcP)+X shr 3)^ and Mask1[x and 7]) shr Shift1[x and 7]], AveP^ do
              begin
                Inc(cR, rgbRed);
                Inc(cG, rgbGreen);
                Inc(cB, rgbBlue);
      ...
      4 : begin
            ...
            for x:=0 to XCount-1 do
            begin
              with Temp.ColorTable[(PByte(intG(SrcP)+X shr 1)^ and Mask4[x and 1]) shr Shift4[x and 1]], AveP^ do
              begin
                Inc(cR, rgbRed);
                Inc(cG, rgbGreen);
                Inc(cB, rgbBlue);
...
(*
function TDIB.SaveImage(var f:file):integer;
var
  res:integer;
begin
  if Empty then Exit;
  ...
  result:=FImage.FBitmapInfo.bmiHeader.biSizeImage;
  GblockWrite(f,FImage.FPBits^,result,res);
end;
*)

function TDIB.SaveImage(f:TStream):integer;
var
  res:integer;
begin
  if Empty then Exit;
  ...
  result:=FImage.FBitmapInfo.bmiHeader.biSizeImage;
  f.Write(FImage.FPBits^,result);
end;




{ Construit un dib de dimensions w * h compatible avec l'affichage courant du PC
}
procedure TDIB.initRgbDib(w,h:integer);
const
  Nbits:integer=0;
var
  form:Tform;
begin
  ...
  begin
    ...
    Nbits:=getDeviceCaps(form.canvas.handle,bitsPixel);
    form.free;
  end;
...
function TDIB.devColor(col: integer): integer;
{  result:=pfRGB(PixelFormat,col and $FF, (col shr 8) and $FF, (col shr 16) and $FF);}
var
  oldC:integer;
begin
  oldC:=pixels[0,0];

  canvas.pixels[0,0]:=col;

  result:=pixels[0,0];
  pixels[0,0]:=oldC;
end;

function TDIB.ColorFromDev(devcol: integer): integer;
{  result:=pfRGB(PixelFormat,col and $FF, (col shr 8) and $FF, (col shr 16) and $FF);}
var
  oldC:integer;
begin
...
procedure TDIB.fillRect(rect: Trect; devcol: integer);
var
  P: PByte;
  x1,x2,y1,y2:integer;
  x,y:integer;
  ...
  case FBitCount of
    1: for x:=x1 to x2 do
       for y:=y1 to y2 do
         begin
           P := @PArrayByte(intG(FTopPBits)+Y*FNextLine)[X shr 3];
           P^ := (P^ and Mask1n[X and 7]) or ((devCol and 1) shl Shift1[X and 7]);
         end;
    ...
    4: for x:=x1 to x2 do
       for y:=y1 to y2 do
         begin
           P := @PArrayByte(intG(FTopPBits)+Y*FNextLine)[X shr 3];
           P^ := (P^ and Mask4n[X and 1]) or ((devCol and 15) shl Shift4[X and 1]);
         end;

    8 : for y:=y1 to y2 do
          fillchar(PArrayByte(intG(FTopPBits)+Y*FNextLine)[X1],(x2-x1+1),devcol);

    16: for y:=y1 to y2 do
        begin
          pw:=PArrayWord(intG(FTopPBits)+Y*FNextLine);
          for x:=x1 to x2 do pw^[x]:=devcol;
        end;

    24:begin
         move(devcol,bgr,sizeof(bgr));
         for y:=y1 to y2 do
         begin
           pBGR:=PArrayBGR(intG(FTopPBits)+Y*FNextLine);
           for x:=x1 to x2 do pBGR^[x]:=BGR;
         end;
       end;

    32:for y:=y1 to y2 do
       begin
         pdw:=PArrayDWord(intG(FTopPBits)+Y*FNextLine);
         for x:=x1 to x2 do pdw^[x]:=devcol;
       end;
  end;

end;

procedure TDIB.fill(devcol: integer);
begin
  fillrect(rect(0,0,width-1,height-1),devcol);
end;

{
  Fait la même chose que CopyRect .
  Déterminant pour FPC : beaucoup plus rapide .
}
procedure TDIB.CopyDibRect(DestRect: Trect; SrcDib: Tdib; srcRect: Trect);
var
  y:integer;
  adsrc,adDest,nbX,nbY,pixSize:integer;
  SrcNextLine: integer;
begin
  ...
  begin
    ...
    if (right>=srcDib.width) then right:=srcDib.width-1;
    if right<left then exit;

    if (top<0) then top:=0;
    if (bottom>=srcDib.height) then bottom:=srcDib.height-1;
  ...
  begin
    if (left<0) then left:=0;
    if (right>=Fwidth) then right:=Fwidth-1;
    if right<left then exit;
  ...
  pixSize:=FbitCount div 8;

  if (srcRect.Right-srcRect.left = destRect.Right-destRect.left) and
     (srcRect.bottom-srcRect.top = destRect.bottom-destRect.top) and
     (pixSize>0) then
  begin
    adsrc:=intG(srcDib.FtopPBits);
    SrcNextLine:=srcDib.FNextLine;

    adDest:=intG(FtopPBits);
    nbX:=srcRect.right-srcRect.left+1;
    nbY:=srcRect.bottom-srcRect.top+1;

    for y:=0 to nbY-1 do
       move(Pbyte(adsrc+(srcRect.top+Y)*srcNextLine +srcRect.left*pixSize)^,
            Pbyte(adDest+(DestRect.top+Y)*FNextLine +destRect.left*pixSize)^,
            nbx*pixSize);

  end
  else canvas.copyRect(DestRect,srcDib.Canvas,srcRect);

end;

procedure TDIB.CopyRectTo(Acanvas: Tcanvas; srcRect: Trect);
var
  SH, SW: Integer;
  res:longbool;
Begin
  ...
  selectClipRgn(Acanvas.Handle,0);  // Pour FPC, il faut déclipper la source, pas la destination
  res:=windows.StretchBlt(Acanvas.Handle, srcRect.Left, srcRect.Top, SW, SH,
    Canvas.Handle, srcRect.Left, srcRect.Top, SW, SH, SrcCopy);
...
function TDIB.extractDib8(x1, y1, x2, y2: integer): Tdib;
var
  i,j:integer;
  p1,p2:PtabOctet;
begin
  if bitCount<>8 then exit;

  result:=Tdib.Create;
  result.ColorTable:=colorTable;
  result.UpdatePalette;
  result.SetSize(x2-x1+1,y2-y1+1,8);

  for j:=y1 to y2 do
    begin
      p1:=scanline[j];
      p2:=result.scanline[j-y1];
      move(p1^[x1],p2^,x2-x1+1);
    end;
end;

function TDIB.extractDib(x1, y1, x2, y2: integer): Tdib;
var
  i,j:integer;
  p1,p2:PtabOctet;
  bCount:integer;
begin
  ...
  result.setSize(x2-x1+1,y2-y1+1,bitCount);
...
procedure Tdib.SaveAsJpeg(st:AnsiString;quality:integer);
var
  hh:Thandle;
  jp:TjpegImage;
  bm0:Tbitmap;
begin
  ...
  try
    ...
    messageCentral('Unable to save '+st);
  end;
end;

procedure Tdib.SaveAsPNG(st:AnsiString);
var
  hh:Thandle;
  png:TpngObject;
  bm0:Tbitmap;
...
Initialization
AffDebug('Initialization DibG',0);
