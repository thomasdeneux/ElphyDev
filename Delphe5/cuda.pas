...
type
  ...
  CUtexObject = longword;
  CUsurfObject = longword;

  CUuuid_st = array[1..16] of char;                                (**< CUDA definition of UUID *)
...
type
  CUipcEventHandle= array[1..CU_IPC_HANDLE_SIZE] of char;

  CUipcMemHandle= array[1..CU_IPC_HANDLE_SIZE] of char;
...
type
  ...
  (
    ...
    CU_AD_FORMAT_SIGNED_INT32   = $a,  // Signed 32-bit integers
...
type
  CUdevprop= record
    ...
    maxThreadsDim : array[1..3] of integer;       // Maximum size of each dimension of a block
    maxGridSize : array[1..3] of integer;         // Maximum size of each dimension of a grid
    sharedMemPerBlock : integer;      // Shared memory available per block in bytes
    totalConstantMemory : integer;    // Constant memory available on device in bytes
...
TCUstreamCallback = procedure( hStream: CUstream; status: CUresult; userData: pointer);stdcall;
{
...
type
  CUDA_MEMCPY2D= record
    srcXInBytes: integer;         {< Source X in bytes }
    srcY: integer;                {< Source Y }
    ...
    srcPitch: integer;            {< Source pitch (ignored when src is array) }

    dstXInBytes: integer;         {< Destination X in bytes }
    dstY: integer;                {< Destination Y }
    ...
    dstPitch: integer;            {< Destination pitch (ignored when dst is array) }

    WidthInBytes: integer;        {< Width of 2D memory copy in bytes }
    Height: integer;              {< Height of 2D memory copy }
  end;

{
 * 3D memory copy parameters
 }
 CUDA_MEMCPY3D = record
    srcXInBytes: integer;         {< Source X in bytes }
    srcY: integer;                {< Source Y }
    srcZ: integer;                {< Source Z }
    srcLOD: integer;              {< Source LOD }
    srcMemoryType: CUmemorytype;  {< Source memory type (host, device, array) }
    srcHost: pointer;             {< Source host pointer }
    srcDevice: CUdeviceptr;      {< Source device pointer }
    srcArray: CUarray;           {< Source array reference }
    ...
    srcPitch: integer;            {< Source pitch (ignored when src is array) }
    srcHeight: integer;           {< Source height (ignored when src is array; may be 0 if Depth==1) }

    dstXInBytes: integer;         {< Destination X in bytes }
    dstY: integer;                {< Destination Y }
    dstZ: integer;                {< Destination Z }
    dstLOD: integer;              {< Destination LOD }
    dstMemoryType: CUmemorytype; {< Destination memory type (host, device, array) }
    dstHost: pointer;              {< Destination host pointer }
    dstDevice: CUdeviceptr;      {< Destination device pointer }
    dstArray: CUarray;           {< Destination array reference }
    ...
    dstPitch: integer;            {< Destination pitch (ignored when dst is array) }
    dstHeight: integer;           {< Destination height (ignored when dst is array; may be 0 if Depth==1) }

    WidthInBytes: integer;        {< Width of 3D memory copy in bytes }
    Height: integer;              {< Height of 3D memory copy }
    Depth: integer;               {< Depth of 3D memory copy }
  end;
...
type
  CUDA_ARRAY3D_DESCRIPTOR = record
    Width: integer;             {< Width of 3D array }
    Height: integer;            {< Height of 3D array }
    Depth: integer;             {< Depth of 3D array }
  ...
  CUDA_RESOURCE_DESC = record
    ...
    case res: integer of
        ...
        struct {
            ...
            integer sizeInBytes;               {< Size in bytes }
        } linear;
        struct {
            CUdeviceptr devPtr;               {< Device pointer }
            CUarray_format format;            {< Array format }
            unsigned int numChannels;         {< Channels per array element }
            integer width;                     {< Width of the array in elements }
            integer height;                    {< Height of the array in elements }
            integer pitchInBytes;              {< Pitch between two rows in bytes }
        } pitch2D;
        struct {
            int reserved[32];
        } reserved;
...
typedef struct CUDA_RESOURCE_VIEW_DESC_st
{
    ...
    integer width;                  {< Width of the resource view }
    integer height;                 {< Height of the resource view }
    integer depth;                  {< Depth of the resource view }
...
{
 * Indicator that the next value in the \p extra parameter to
 * ::cuLaunchKernel will be a pointer to a integer which contains the
...
CUresult CUDAAPI cuDeviceTotalMem(integer *bytes, CUdevice dev);
...
CUresult CUDAAPI cuCtxSetLimit(CUlimit limit, integer value);
...
CUresult CUDAAPI cuCtxGetLimit(integer *pvalue, CUlimit limit);
...
CUresult CUDAAPI cuModuleGetGlobal(CUdeviceptr *dptr, integer *bytes, CUmodule hmod, const char *name);
#endif /* __CUDA_API_VERSION >= 3020 }
...
CUresult CUDAAPI cuMemGetInfo(integer *free, integer *total);
...
CUresult CUDAAPI cuMemAlloc(CUdeviceptr *dptr, integer bytesize);
...
CUresult CUDAAPI cuMemAllocPitch(CUdeviceptr *dptr, integer *pPitch, integer WidthInBytes, integer Height, unsigned int ElementSizeBytes);
...
CUresult CUDAAPI cuMemGetAddressRange(CUdeviceptr *pbase, integer *psize, CUdeviceptr dptr);
...
CUresult CUDAAPI cuMemAllocHost(void **pp, integer bytesize);
#endif /* __CUDA_API_VERSION >= 3020 }
...
CUresult CUDAAPI cuMemHostAlloc(void **pp, integer bytesize, unsigned int Flags);
...
CUresult CUDAAPI cuMemHostRegister(void *p, integer bytesize, unsigned int Flags);
...
CUresult CUDAAPI cuMemcpy(CUdeviceptr dst, CUdeviceptr src, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyPeer(CUdeviceptr dstDevice, CUcontext dstContext, CUdeviceptr srcDevice, CUcontext srcContext, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyHtoD(CUdeviceptr dstDevice, const void *srcHost, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyDtoH(void *dstHost, CUdeviceptr srcDevice, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyDtoD(CUdeviceptr dstDevice, CUdeviceptr srcDevice, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyDtoA(CUarray dstArray, integer dstOffset, CUdeviceptr srcDevice, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyAtoD(CUdeviceptr dstDevice, CUarray srcArray, integer srcOffset, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyHtoA(CUarray dstArray, integer dstOffset, const void *srcHost, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyAtoH(void *dstHost, CUarray srcArray, integer srcOffset, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyAtoA(CUarray dstArray, integer dstOffset, CUarray srcArray, integer srcOffset, integer ByteCount);
...
CUresult CUDAAPI cuMemcpyAsync(CUdeviceptr dst, CUdeviceptr src, integer ByteCount, CUstream hStream);
...
CUresult CUDAAPI cuMemcpyPeerAsync(CUdeviceptr dstDevice, CUcontext dstContext, CUdeviceptr srcDevice, CUcontext srcContext, integer ByteCount, CUstream hStream);
#endif /* __CUDA_API_VERSION >= 4000 }
...
CUresult CUDAAPI cuMemcpyHtoDAsync(CUdeviceptr dstDevice, const void *srcHost, integer ByteCount, CUstream hStream);
...
CUresult CUDAAPI cuMemcpyDtoHAsync(void *dstHost, CUdeviceptr srcDevice, integer ByteCount, CUstream hStream);
...
CUresult CUDAAPI cuMemcpyDtoDAsync(CUdeviceptr dstDevice, CUdeviceptr srcDevice, integer ByteCount, CUstream hStream);
...
CUresult CUDAAPI cuMemcpyHtoAAsync(CUarray dstArray, integer dstOffset, const void *srcHost, integer ByteCount, CUstream hStream);
...
CUresult CUDAAPI cuMemcpyAtoHAsync(void *dstHost, CUarray srcArray, integer srcOffset, integer ByteCount, CUstream hStream);
...
CUresult CUDAAPI cuMemsetD8(CUdeviceptr dstDevice, unsigned char uc, integer N);
...
CUresult CUDAAPI cuMemsetD16(CUdeviceptr dstDevice, unsigned short us, integer N);
...
CUresult CUDAAPI cuMemsetD32(CUdeviceptr dstDevice, unsigned int ui, integer N);
...
CUresult CUDAAPI cuMemsetD2D8(CUdeviceptr dstDevice, integer dstPitch, unsigned char uc, integer Width, integer Height);
...
CUresult CUDAAPI cuMemsetD2D16(CUdeviceptr dstDevice, integer dstPitch, unsigned short us, integer Width, integer Height);
...
CUresult CUDAAPI cuMemsetD2D32(CUdeviceptr dstDevice, integer dstPitch, unsigned int ui, integer Width, integer Height);
...
CUresult CUDAAPI cuMemsetD8Async(CUdeviceptr dstDevice, unsigned char uc, integer N, CUstream hStream);
...
CUresult CUDAAPI cuMemsetD16Async(CUdeviceptr dstDevice, unsigned short us, integer N, CUstream hStream);
...
CUresult CUDAAPI cuMemsetD32Async(CUdeviceptr dstDevice, unsigned int ui, integer N, CUstream hStream);
...
CUresult CUDAAPI cuMemsetD2D8Async(CUdeviceptr dstDevice, integer dstPitch, unsigned char uc, integer Width, integer Height, CUstream hStream);
...
CUresult CUDAAPI cuMemsetD2D16Async(CUdeviceptr dstDevice, integer dstPitch, unsigned short us, integer Width, integer Height, CUstream hStream);
...
CUresult CUDAAPI cuMemsetD2D32Async(CUdeviceptr dstDevice, integer dstPitch, unsigned int ui, integer Width, integer Height, CUstream hStream);
 ...
 *   value in \p extra will be a pointer to a integer containing the
 *   size of the buffer specified with ::CU_LAUNCH_PARAM_BUFFER_POINTER;
 *
...
CUresult CUDAAPI cuTexRefSetAddress(integer *ByteOffset, CUtexref hTexRef, CUdeviceptr dptr, integer bytes);
...
CUresult CUDAAPI cuTexRefSetAddress2D(CUtexref hTexRef, const CUDA_ARRAY_DESCRIPTOR *desc, CUdeviceptr dptr, integer Pitch);
#endif /* __CUDA_API_VERSION >= 3020 }
...
{
 * \brief Sets the maximum anistropy for a texture reference
 *
 * Specifies the maximum aniostropy \p maxAniso to be used when reading memory through
...
{
 * \brief Gets the maximum anistropy for a texture reference
 *
 * Returns the maximum aniostropy in \p pmaxAniso that's used when reading memory through
 * the texture reference \p hTexRef. 
 *
 ...
 *
 * - ::CUDA_TEXTURE_DESC::maxAnisotropy specifies the maximum anistropy ratio to be used when doing anisotropic filtering. This value will be
...
CUresult CUDAAPI cuGraphicsResourceGetMappedPointer(CUdeviceptr *pDevPtr, integer *pSize, CUgraphicsResource resource);
#endif /* __CUDA_API_VERSION >= 3020 }
...
CUresult CUDAAPI cuTexRefSetAddress2D_v2(CUtexref hTexRef, const CUDA_ARRAY_DESCRIPTOR *desc, CUdeviceptr dptr, integer Pitch);
