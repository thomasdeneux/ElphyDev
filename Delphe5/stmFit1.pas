...
type
  TFit=class(Tfunction)
       private
         ...
         NumPar:array of integer;{ Numpar[i] donne la position dans valeur du
         ...
         FSigmaA: array of Float;   {array[1..maxPara] of float;
                                    en sortie du fit, incertitudes sur para}

         FfitProp1: TFitProp1;

         function getSigmaA(i: integer): float;
         procedure setSigmaA(i:integer; w:float);

         procedure setNbVar(n:integer);override;

         function FitProp1: TFitProp1;
       public
         ...
         property SigmaA[i:integer]: float read getSigmaA write setSigmaA;
       end;
...
begin
  ...
  begin
    ...
    fitX:=v;
    refObjet(fitX);
    if assigned(FfitProp1) then FitProp1.update;
  end;
end;
...
function F0fit(z:float; p :PtabFloat1):float;
  var
    i:integer;
  begin
    with actualFit do
    begin
      valeur[numvar]:=z;
      for i:=1 to nbparaFit do valeur[NumPar[i]]:=p[i];
      F0fit:=evaluer(U0,Fvaleur);
    end;
  end;

function F0fitPG(z:float; p: PtabFloat1):float;
  var
    i:integer;
  begin
    with actualFit do
...
Procedure Derivee0fit(var z:float; p: PtabFloat1;
  ...
  begin
    ...
    begin
      valeur[numvar]:=z;
      for i:=1 to nbparaFit do valeur[NumPar[i]]:=p[i];
      for i:=1 to nbParafit do deriv[i]:=evaluer(U1[i],Fvaleur);
    end;
  end;
...
function Tfit.fit:boolean;
  ...
  function FitU:boolean;
    var
      i:integer;
      paraClFit: array of boolean;

    begin
      setlength(paraFit,nbvar+1);
      setlength(numPar,nbvar+1);
      setlength(ParaClFit,nbvar+1);
      ...
      begin
        ...
        for i:=1 to nbvar do
          if (i<>numvar) and not FitparaCl[i] then
            begin
              inc(nbparaFit);
              numPar[nbparaFit]:=i;                  // numéro dans la liste originale. Commence à 1
              paraFit[nbParaFit]:=valeur[i];
            end;
        for i:=1 to nbparaFit do
          begin
            detruireArbre(U1[i]);
            U1[i]:=derivee(U0,numpar[i]-1);          // derivee utilise des numéros commençant à 0
            //Affdebug(AffArbre0(U1[i],Fnomvar),101)
          end;

        fillchar(paraClfit[0],length(paraClfit),0);
  ...
  begin
    ...
    if numModel>0 then
      ...
      begin
        nbvar:= n+1;
        if initialize then
          initF(pxFTR,pyFTR,@Fvaleur[1],FitNbpt,n);
...
begin
  try
    ...
    if (fitNbpt>2) and (sqX>0) then residualVariance:=1/(FitNbpt-2)*(sqY-sqr(sqXY)/sqX);
    result:=true;
  except
    result:=false;
  end;
...
procedure Tfit.CompressParam(v:float);
 ...
 var
  ...
  begin
    ...
    if proche(valeur[3],valeur[5],v) then
      begin
        valeur[2]:=valeur[2]+valeur[4];
        valeur[4]:=0;
        if numModel>20 then valeur[5]:=Vnul else valeur[5]:=0;
      end;

    if (Md in [3,6]) then
      begin
        if proche(valeur[5],valeur[7],v) then
          begin
            valeur[4]:=valeur[4]+valeur[6];
            valeur[6]:=0;
            if numModel>20 then valeur[7]:=Vnul else valeur[7]:=0;
          end;
...
function Tfit.getSigmaA(i: integer): float;
begin
  if (i>=1) and (i<=length(FsigmaA))
    then result:= FsigmaA[i-1]
    else result:=0;
end;


procedure Tfit.setSigmaA(i: integer; w: float);
begin
  if (i>=1) and (i<=length(FsigmaA))
    then FsigmaA[i-1]:=w;
end;

function TFit.FitProp1: TFitProp1;
begin
  if not assigned(FFitProp1) then FFitProp1:= TFitProp1.create(nil);
  result:= FFitProp1;
end;
...
procedure TFit.setNbVar(n: integer);
var
  old:integer;
begin
  inherited;
  old:=length(FsigmaA);
  setlength(FsigmaA,n);
  if n>old then fillchar(FsigmaA[old],(n-old)*sizeof(float),0);
end;
