...
function affarbre0(var rac:ptelem; nomVar:TstringList): AnsiString;

Function evaluer(var rac:ptelem; valeur:tabval):float;
...
function numeroVariable(st:AnsiString; nomVar:TstringList): integer;

procedure verifierListe(list:TstringList;var lig,col,error:integer);
procedure CreerArbreListe(var rac:ptelem;texte:TstringList; nomVar:TstringList);
...
function Fheaviside(x:float):float;
begin
  if x<0 then result:=0
  else result:= 1;
end;

function Fdirac (x:float):float;
begin
  if x=0
    then result:=1E200
    else result:=0;
...
procedure AddFunction(name: AnsiString; f: typefonc1; derivee: AnsiString);
begin
  FunctionList.addObject(name, @f);
  DerivList.AddObject(derivee,nil);
end;

function calculDeriv(st: AnsiString): pointer;
var
  pc:integer;
  rac:ptelem;
  nomvar: TstringList;
...
begin
  ...
  DerivList:=Tstringlist.create;

  AddFunction( 'SIN'    , Fsin, 'cos(x)');
  AddFunction( 'COS'    , Fcos, '-sin(x)');

  AddFunction( 'TAN'    , Ftg, '1+sqr(tan(x))');
  AddFunction( 'TG'     , Ftg, '1+sqr(tan(x))');

  AddFunction( 'COT'  , Fcotg, '-1-sqr(cotan(x))');
  AddFunction( 'COTG'   , Fcotg, '-1-sqr(cotan(x))');
  AddFunction( 'COTAN'  , Fcotg, '-1-sqr(cotan(x))');

  AddFunction( 'ATAN' , Farctg, '1/(1+sqr(x))');
  AddFunction( 'ARCTAN' , Farctg, '1/(1+sqr(x))');
  AddFunction( 'ARCTG'  , Farctg, '1/(1+sqr(x))');

  AddFunction( 'ARCSIN' , Farcsin, '1/sqrt(1-sqr(x))');
  AddFunction( 'ARCCOS' , Farccos, '-1/sqrt(1-sqr(x))  ');

  AddFunction( 'EXP'    , Fexp, 'exp(x)');
  AddFunction( 'LN'     , Flog, '1/x');
  AddFunction( 'LOG'    , Flog, '1/x  ');


  AddFunction( 'SQR'    , Fsqr, '2*x');
  AddFunction( 'SQRT'   , Fsqrt, '1/2/sqrt(x)');
  AddFunction( 'SH'     , Fsh, 'ch(x)');
  AddFunction( 'CH'     , Fch, 'sh(x)');
  AddFunction( 'TH'     , Fth, '1-sqr(th(x))');

  AddFunction( 'DIRAC', Fdirac, '');
  AddFunction( 'HEAVISIDE', Fheaviside, 'DIRAC(x)');
  AddFunction( 'ABS'    , Fabs, 'HEAVISIDE(x)*2-1');

  AddFunction( 'erf'    , Ferf, '2/sqrt(PI)*exp(-x*x)');

  with DerivList do
  for i:=0 to count-1 do
    objects[i]:= CalculDeriv(strings[i]);
end;
...
function numeroVariable(st:AnsiString; nomVar:TstringList): integer;
begin
  result:= nomvar.IndexOf(st);
  if result<0 then result:= nomvar.Add(st);
end;
...
procedure CreerArbre(var rac:ptelem;chdef:AnsiString;var pc:integer; nomVar:TstringList);
  var
    i:integer;
    st:AnsiString;
      ...
      if chdef[pc] in lettre then
        begin
          ...
          if chdef[pc]<>'(' then
            begin
              rac^.genre:=vari;
              rac^.num:=NumeroVariable(st,nomvar);
              dec(pc);
            end
          else
            begin
...
begin
  ...
  begin
    case rac^.genre of
      op :begin
            if assigned(rac^.g) and (rac^.g.genre=op) then result:=result+'(';
            result:=result + Affarbre0(rac^.g,nomvar);
            if assigned(rac^.g) and (rac^.g.genre=op) then result:=result+')';
            result:=result+rac^.opnom;
            if assigned(rac^.d) and (rac^.d.genre=op) then result:=result+'(';
            result:=result + Affarbre0(rac^.d,nomvar);
            if assigned(rac^.d) and (rac^.d.genre=op) then result:=result+')';
          end;

      nbr:  result:=result+ecrireel(rac^.vnbr);

      vari:  result:=result+nomvar[rac^.num];

      fonc:begin
             result:=result+NomFonc1(rac^.f);
             result:=result+'(';
             result:= result+Affarbre0(rac^.arg,nomvar);
             result:=result+')';
           end;
    end;
  end
...
function PCopyAndReplace(r,r1:ptelem):ptelem;
var
  a:ptelem;
begin
  ...
  begin
    if r^.genre=vari then a:= Pcopie(r1)
    else
    begin
      new(a);
      move(r^,a^,sizeof(typelem));
      case r^.genre of
        op: begin
              a^.g:=PcopyAndReplace(r^.g,r1);
              a^.d:=PcopyAndReplace(r^.d,r1);
            end;
        fonc: a^.arg:=PcopyAndReplace(r^.arg,r1);
      end;
    end;
    result:= a;
  end;
...
function BuildDeriv(f: typeFonc1 ; rac:ptElem): ptElem;
var
  n:integer;
begin
  n:=FunctionList.IndexOfObject(@f);
  result:= PcopyAndReplace(ptElem(DerivList.Objects[n]),rac);
end;
...
procedure CreerArbre1(var rac:ptelem;chdef:AnsiString;var pc:integer;
          var nomVar:TstringList; exp:TstringList);
  var
    i:integer;
    st:AnsiString;
...
begin
  ...
  for i:=0 to count-1 do
    begin
      ...
      inc(pc);
      CreerArbre1(rac,st,pc,nomVar,exp);
...
begin
  ...
  compilerListe(Texte,exp,nomvar);
...
Initialization
AffDebug('Initialization Darbre1',0);
initFunctions;
