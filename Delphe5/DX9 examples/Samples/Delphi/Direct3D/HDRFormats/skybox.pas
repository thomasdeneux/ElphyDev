 ...
 *----------------------------------------------------------------------------*
 *  $Id: skybox.pas,v 1.11 2007/02/05 22:21:07 clootie Exp $
 *----------------------------------------------------------------------------*)
//-----------------------------------------------------------------------------
...
type
  ...
  public
    ...
    {$IFNDEF FPC}
    function OnCreateDevice(const pd3dDevice: IDirect3DDevice9; fSize: Single; pEnvMap: IDirect3DCubeTexture9; strEffectFileName: WideString): HRESULT; overload;{$IFDEF SUPPORTS_INLINE} inline;{$ENDIF}
    {$ENDIF}

    procedure OnResetDevice(const pBackBufferSurfaceDesc: TD3DSurfaceDesc);
    procedure Render(const pmWorldViewProj: TD3DXMatrix);
...
begin
  ...
  Result:= OnCreateDevice(pd3dDevice, fSize, pCubeTexture, strEffectFileName);
end;
...
function CSkybox.OnCreateDevice(const pd3dDevice: IDirect3DDevice9; fSize: Single; pEnvMap: IDirect3DCubeTexture9; strEffectFileName: WideString): HRESULT; 
begin
  Result := OnCreateDevice(pd3dDevice, fSize, pEnvMap, PWideChar(strEffectFileName));
end;
...
// Desc: 
//-----------------------------------------------------------------------------
procedure CSkybox.Render(const pmWorldViewProj: TD3DXMatrix);
var
