...
type
  ...
  public
    ...
    function ExecuteUpdatePreparedGS(keys:string): Integer; virtual;
  ...
  protected
    ...
    function PrepareSQLQuery(keys:string): string; virtual;
  ...
  public
    ...
    function ExecuteUpdatePreparedGS(keys:string): Integer; override;
...
function TZAbstractPreparedStatement.ExecuteUpdatePreparedGS(keys:string): Integer;
begin
  Result := -1;
  RaiseUnsupportedException;
end;
...
function TZEmulatedPreparedStatement.PrepareSQLQuery(keys:string): string;
var
  I: Integer;
  ParamIndex: Integer;
  Tokens: TStrings;
...
function getIdent(st:string):string;
const
...
procedure Process1(st:string);
begin
  st:=getIdent(st);
  if st<>'' then list.Add(st);
end;

function test1(st:string): boolean;  // true if must be removed
begin
  st:=getIdent(st);
  result:= (st<>'') and
           (
...
function TZEmulatedPreparedStatement.ExecuteUpdatePreparedGS(keys:string): Integer;
begin
