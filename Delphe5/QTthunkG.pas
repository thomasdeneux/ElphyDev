...
function Call16BitRoutine(Name: Pchar; DllHandle: THandle16;
  Convention: TConvention; Args: array of const;
  ArgSizes: array of Integer): Longint;


function Call16BitG( procAd:pointer;Args: array of const;
                     ArgSizes: array of Integer): Longint;

{ fonction identique à la précédente mais
    - on donne l'adresse obtenue auparavant avec getProcAddress16
    - la convention d'appel est toujours Pascal
  de cette façon, l'appel est 15 fois plus rapide.
  Exemple, on passe de 107 µs à 7.7 µs dans getTimer2. Valeur voisine de celle
  obtenue dans le programme 16 bits

    - la routine originale oublie curieusement les nombres réels. La nouvelle
  routine autorise le passage par valeur des Extended

    - pour transmettre un vrai pointeur, le plus simple est de le transtyper
    en longint
}

function InitQTthunkG:boolean;
...
function LoadLib16(LibFileName: String): THandle;
begin
  {messageCentral(LibFileName);}
  Result := LoadLibrary16(PChar(LibFileName));
  if Result < HInstance_Error then
    begin
      {$IFDEF DEBUG}
      messageCentral('Unable to load '+libFileName);
      {$ENDIF}
      result:=0;
    end;
...
function CallQT_ThunkG: Longint;
var
  EatStackSpace: String[$3C];
begin
  //Ensure buffer isn't optimised away
...
var
  ...
  var
    i:integer;
    x:extended;
    tab:array[1..5] of word absolute x;
  begin
    ...
    case Args[Param].VType of
      ...
      vtExtended:
      begin
        x:=args[param].Vextended^;
        for i:=0 to 4 do Move(tab[5-i],ArgBuf[ArgByteCount+2*i], SizeOf(Word));
        Inc(ArgByteCount, SizeOf(Word)*5);
      end;
...
begin
  ...
  {if @QT_Thunk = nil then
    messageCentral('Flat thunks only supported under Windows 95-98');}
end;

{$else}
Initialization
AffDebug('Initialization QTthunkG',0);
