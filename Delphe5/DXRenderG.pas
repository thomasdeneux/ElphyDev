...
function dxrScanLine(const Surface: TDXR_Surface; y: DWORD): Pointer;
begin
  Result := Pointer(intG(Surface.Bits)+Surface.Pitch*Integer(y));
end;
      ...
      begin
        ...
        begin
          asm
            jmp @@EndCode
          @@StartCode:
            mov al,byte ptr [offset offset _null]{}
            @@Col1:
            mul byte ptr [offset offset _null]   {}
            @@Col2:
            mov byte ptr [offset offset _null],ah{}
            @@Dest:
          @@EndCode:
            {$I DXRenderG.inc}
            {  @@Col1  }
  ...
  begin
    ...
    begin
      if States.EnableDrawLine and (1 shl (y and 31))<>0 then
        if PInteger(IntG(@y_nx1)+4)^<PInteger(IntG(@y_nx2)+4)^ then
        begin
          drawline(
            PInteger(IntG(@y_nx1)+4)^, PInteger(IntG(@y_nx2)+4)^, y,
            y_ntex1, y_ntex2,
            y_nc1, y_nc2
          );
        end else if PInteger(IntG(@y_nx1)+4)^>PInteger(IntG(@y_nx2)+4)^ then
        begin
          drawline(
            PInteger(IntG(@y_nx2)+4)^, PInteger(IntG(@y_nx1)+4)^, y,
            y_ntex2, y_ntex1,
            y_nc2, y_nc1
          );
        end;
...
Initialization
AffDebug('Initialization DXRenderG',0);
  ReadCPUID;
  Init;
  InitCosinTable;
finalization
