...
const
  ATOM_HTMLHELP_API_ANSI: Pansichar = #0#0#0#14;
  ATOM_HTMLHELP_API_UNICODE: Pansichar = #0#0#0#15;
...
begin
  ...
  begin
    HelpChecked := True;
    try
      HelpModule := GLoadLibrary(HELPDLL);
    except
      HelpModule := 0;
    end;
...
function HtmlHelp(hwndCaller: HWND; pszFile: PAnsiChar; uCommand: UINT;
  dwData: DWORD): HWND;
begin
...
Initialization
AffDebug('Initialization HTMLhelp1',0);
  HelpModule := 0;
  HH := nil;
  HelpChecked := False;
finalization
