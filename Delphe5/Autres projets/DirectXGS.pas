...
{ L'unité originale initialise toutes les DLL et appelle DisableFPUExceptions .
  Avec les procédures qui suivent, on peut initialiser seulement ce qui est
  nécessaire.

  Est-il nécessaire d'appeler DisableFPUExceptions ? Il vaut mieux activer
  cette méthode seulement pendant l'animation.

  Il manque le chargement de d3dim.dll . C'est peut-être un oubli.
}
procedure InitializeDDraw;
procedure InitializeD3DRM;
procedure InitializeDinput;
procedure InitializeDplay;
procedure InitializeDsound;
procedure InitializeDXfile;
...
function D3DVal(val: variant) : DXfloat;
function D3DDivide(a,b: double) : DXfloat;
function D3DMultiply(a,b: double) : DXfloat;
 ...
 * The DXfloat values must be in the range 0..1
 *)

function D3DRGB(r, g, b: DXfloat) : TD3DColor;
function D3DRGBA(r, g, b, a: DXfloat) : TD3DColor;
...
type
  TD3DClipStatus = packed record
    ...
    minx, maxx : DXfloat; (* X extents *)
    miny, maxy : DXfloat; (* Y extents *)
    minz, maxz : DXfloat; (* Z extents *)
  end;
...
procedure InitializeDDraw;
Begin
  If Not IsNTandDelphiRunning Then LoadDDrawDLL;
end;

procedure InitializeD3DRM;
Begin
  If Not IsNTandDelphiRunning Then LoadD3DRMDLL;
end;

procedure InitializeDinput;
Begin
  If Not IsNTandDelphiRunning Then LoadDinputDLL;
end;


procedure InitializeDplay;
Begin
  If Not IsNTandDelphiRunning Then LoadDplayDLL;
end;

procedure InitializeDsound;
Begin
  If Not IsNTandDelphiRunning Then LoadDsoundDLL;
end;

procedure InitializeDXfile;
Begin
  If Not IsNTandDelphiRunning Then LoadDXfileDLL;
end;
