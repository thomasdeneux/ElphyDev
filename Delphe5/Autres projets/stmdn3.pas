...
type
  ...
  TdenseNoise=class(Trevcor)
              ...
              public
                ...
                function Von(x,y:integer):integer;
...
procedure TdenseNoise.calculBruit;
var
  i: integer;
begin
  setLength(bb,lgBB);
...
begin
  ...
  grid.addObject(obvis);
  grid.addObject(obvis2);
...
function TdenseNoise.Von(x,y:integer):integer;
var
  k,decal:integer;
begin
  k:=x+nx*y;
...
begin
  if affStim and ObON then Grid.FonScreen:=true;
  if affControl and ObON then Grid.FonControl:=true;
end;
    ...
    if not Ferror then
      ...
      begin
        ...
        for i:=0 to Klist.count-1 do
          ...
          if First
            then data^.addE(t,value*bb[(decal1+t) mod LgBB])
            else data^.addE(t,value*bb[(decal1+t) mod LgBB]
                            *bb[(decal2+t) mod LgBB]);
      end;
...
begin
  verifierObjet(pu);
  with TdenseNoise(pu) do result:=lgBB;
end;
