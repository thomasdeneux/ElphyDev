unit Direct3D9G;
// Same as Direct3D9.pas but with dynamic linking (see InitDirect3D9lib )
...
const
  ...
  D3DBLEND_SRCCOLOR2         = 16;     //Ex
  {$EXTERNALSYM D3DBLEND_SRCCOLOR2}
  D3DBLEND_INVSRCCOLOR2      = 17;     //Ex
...
const
  ...
  D3DTEXF_CONVOLUTIONMONO = 8;    // Ex : Convolution filter for monochrome textures
...
type
  _D3DFORMAT = (
    ...
    D3DFMT_D32_LOCKABLE         = 84,  //ex
   {$EXTERNALSYM D3DFMT_D32_LOCKABLE}
    D3DFMT_S8_LOCKABLE          = 85,  //ex
  ...
  _D3DSWAPEFFECT = (
    ...
    D3DSWAPEFFECT_OVERLAY       = 4,  // ex
    D3DSWAPEFFECT_FLIPEX        = 5   // ex
  );
...
const
  ...
  {$EXTERNALSYM D3DUSAGE_DYNAMIC}
    ...
    D3DUSAGE_NONSECURE          = $00800000;  //ex
 ...
 ***************************************************************************)
  ...
  PD3DComposeRectsOp = ^TD3DComposeRectsOp;                           // Ex begin
  _D3DCOMPOSERECTSOP =
  (
...
type
  ...
  TD3DDisplayRotation = D3DDISPLAYROTATION;                           //ex end
...
const
  ...
  {$EXTERNALSYM D3DCAPS2_CANAUTOGENMIPMAP}
   ...
   D3DCAPS2_CANSHARERESOURCE     = $80000000;    //ex
  ...
  D3DPMISCCAPS_POSTBLENDSRGBCONVERT       = $00200000; { Ex      Indicates device can perform conversion to sRGB after blending. }
  ...
  D3DPTFILTERCAPS_CONVOLUTIONMONO       = $00040000; // Min and Mag for the convolution mono filter   EX
 ...
 ****************************************************************************)
  ...
  {$EXTERNALSYM D3DCREATE_NOWINDOWCHANGES}
    ...
    // Disable multithreading for software vertex processing                 //ex
...
(* NO_DRIVERVERSION will not fill out the DriverVersion field, nor will the
  ...
  // D3DENUM_WHQL_LEVEL                      = $00000002;  // Obsolète en ex
 ...
 ****************************************************************************)
  ...
  D3DPRESENT_DONOTFLIP                   = $00000004;  //ex
  {$EXTERNALSYM D3DPRESENT_DONOTFLIP}
  D3DPRESENT_FLIPRESTART                 = $00000008;  //ex
  {$EXTERNALSYM D3DPRESENT_FLIPRESTART}


  D3DPRESENT_VIDEO_RESTRICT_TO_MONITOR   = $10;
  D3DPRESENT_UPDATEOVERLAYONLY           = $20;
  D3DPRESENT_HIDEOVERLAY                 = $40;
  D3DPRESENT_UPDATECOLORKEY              = $80;
  D3DPRESENT_FORCEIMMEDIATE              = $100;
...
// pointer we should declare result not as interface but as usual pointer

var
 _Direct3DCreate9: function (SDKVersion: LongWord): Pointer; stdCall;{ name 'Direct3DCreate9';}
  Direct3DCreate9Ex: function (SDKVersion: LongWord; out d3d9ex: IDirect3D9Ex): HRESULT; stdcall;
...
function InitDirect3D9lib: boolean;
 ...
 ***************************************************************************)
 ...
 // Le traducteur a oublié les &0xFF de la macro !!
 // Donc, si les paramètres r g ou b débordent, il y a mélange des couleurs.
...
begin
  ...
  hh:=loadLibrary( Direct3D9dll );
  result:=(hh<>0);
  if not result then exit;

  D3DPERF_BeginEvent:= getProc(hh,'D3DPERF_BeginEvent');
  D3DPERF_EndEvent:= getProc(hh,'D3DPERF_EndEvent');
  D3DPERF_SetMarker:= getProc(hh,'D3DPERF_SetMarker');
  D3DPERF_SetRegion:= getProc(hh,'D3DPERF_SetRegion');
  D3DPERF_QueryRepeatFrame:= getProc(hh,'D3DPERF_QueryRepeatFrame');

  D3DPERF_SetOptions:= getProc(hh,'D3DPERF_SetOptions');
  D3DPERF_GetStatus:=  getProc(hh,'D3DPERF_GetStatus');
  _Direct3DCreate9:=   getProc(hh,'Direct3DCreate9');
  Direct3DCreate9ex:=  getProc(hh,'Direct3DCreate9Ex');
end;
