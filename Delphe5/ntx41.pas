...
// static char* ntxh_header = "$Header: x:/engineer/intime.400/nt/include/rcs/ntx.h 1.41 2009/11/13 04:18:43Z cmain Engineer $";

(*
 * Warn about mixing RT anbd RTE subsystems

 * We skip UNICODE definitions
...
(*
 * NTX_ALL_LOCAL_LOCATIONS is an NTXLOCATION referring to all
 * local locations currently defined in the configuration.
 * Used to start/stop all RtNodes at once.
 *)
  NTX_ALL_LOCAL_LOCATIONS      = NTXLOCATION(-2);
 ...
 *)
  ...
  RESPONSE_MESSAGE 				=$0040;
   ...
   NTX_WAIT_FOR_IOPROXY            		= 1;
   NTX_WAIT_FOR_IOSERVICE          		= 2;
...
// NTXLOCATIONINFO Structure
  NTXLOCATIONINFO = record
                       LocationName: Pansichar;
                       ClassName: Pansichar;
                     end;

  LPNTXLOCATIONINFO = ^NTXLOCATIONINFO ;
...
(*
 * Returns INtime version strings
 *)
  ntxGetINtimeVersion: function ( productVersion: Pansichar; versionSize: Dword;
				 updateVersion: Pansichar;  updateSize: Dword): integer;cdecl;

(*
 * Returns INtime version as an integer
 *)
  ntxGetINtimeVersionNumber: function: integer;cdecl;
...
ntxGetLocationByName: function(const LocationName:Pansichar):NTXLOCATION ;cdecl;
...
(*
 * Returns information associated with the specified location handle
 *
 * If the client writes to the returned pointers, results are unpredictable
 * (but almost certainly BAD).
 *)
ntxGetLocationInfo:function( Location:NTXLOCATION; pLocationInfo: LPNTXLOCATIONINFO ):NTXSTATUS;cdecl;
...
(*
 * Sends Start command to INtime Kernel Service to start the INtime kernel on
 * the specified local NTXLOCATION.
 *
 * Returns E_OK if the start command is successfully sent.
 *)

ntxStartLocalRtNode: function(Location: NTXLOCATION ):NTXSTATUS;cdecl;

(*
 * Sends Stop command to INtime Kernel Service to stop the INtime kernel on
 * the specified local NTXLOCATION.
 *
 * Returns E_OK if the stop command is successfully sent.
 *)

ntxStopLocalRtNode: function(Location: NTXLOCATION ):NTXSTATUS;cdecl;
...
(*
 * Sends a TERMINATE message to the given process
 *)
ntxTerminateRtProcess: function( hRtProcess: NTXHANDLE): NTXSTATUS;cdecl;

(*
 * Delete an RT process
 *
 * Returns E_OK on success, else check ntxGetLastRtError
 *)
ntxDeleteRtProcess: function( hProcess: NTXHANDLE): NTXSTATUS;cdecl;
...
(*
 * Wait for the IO Service and/or IO Proxy to initialize
 *
 * Fails if NTXLOCATION is invalid, not defined, or
 * not reachable (E_LOCATION).
 *
 * Returns error status on failure (check ntxGetLastRtError)
 *)

ntxWaitForIoService: function(hLoc: NTXLOCATION; dwFlags: DWORD; dwMilliseconds: DWORD):NTXSTATUS;cdecl;
...
begin
  ...
  ntxGetINtimeVersion:= getProc(hh,'ntxGetINtimeVersion');
  ntxGetINtimeVersionNumber:= getProc(hh,'ntxGetINtimeVersionNumber');
  ...
  ntxGetLocationInfo:= getProc(hh,'ntxGetLocationInfo');

  ntxGetLocationOfRtObject:=getProc(hh,'ntxGetLocationOfRtObject');
  ntxStartLocalRtNode:= getProc(hh,'ntxStartLocalRtNode');
  ntxStopLocalRtNode:= getProc(hh,'ntxStopLocalRtNode');
...
begin
  ...
  ntxDeleteRtProcess:= getProc(hhex,'ntxDeleteRtProcess');
end;
...
Initialization
AffDebug('Initialization ntx41',0);
