...
uses classes,sysutils,util1,math;
...
type
  ...
  TUIC2entry=record
               ZdistanceN,ZdistanceD:longWord;
               CreationDate,creationTime:longword;
               ModificationDate,ModificationTime:longword;
             end;

  TstkProperties=record
                    Zdistance:single;
                    CreationDate,creationTime:longword;
                    ModificationDate,ModificationTime:longword;
                    waveLength:single;
  ...
  TIFD=       record
                nbDirEntries:word;
                DirEntry:array[0..10000] of TdirEntry;
              end;


  TstkFile=   class
                stFile:AnsiString;
                DirEntry:array of TdirEntry;
                File0:TfileStream;
                ...
                BitsPerSample: TarrayOfInteger;
                SamplesPerPixel: integer;

                RowsPerStrip: integer;
                StripByteCounts: TarrayOfinteger;
                StripOffsets: TarrayOfinteger;

                stkCount:integer;
                stk:Array of TstkProperties;
                ...
                function Init(stF:AnsiString):boolean;
                procedure AnalyzeEntry(var entry:TdirEntry);


                function loadBuffer(num:integer;var bufsize:integer):pointer;
...
procedure TstkFile.AnalyzeEntry(var entry:TdirEntry);
var
  i:integer;

function TagInt:longWord;
begin
  with entry do
  case tagType of
    TIFF_BYTE :  result:=byte(tagValue);
    TIFF_SHORT : result:=word(tagValue);
    TIFF_LONG :  result:=tagValue;
    else messageCentral('TagInt : type='+Istr(tagType));
  end;
end;

function tagRational:float;
var
  res:integer;
  w:Trational;
begin
  with entry do
  begin
    fileStream.seek(tagValue,soFromBeginning);
    fileStream.Read(w,sizeof(w));
    if (w.denominator<>0)
      then result:=w.numerator / w.denominator
      else result:=0;
  end;
end;

procedure readIntArray(var w:TarrayOfInteger);
var
  dum:array of smallint;
  i,res:integer;
begin
  with entry do
  begin
    setLength(w,tagCount);

    case tagType of
      TIFF_SHORT: begin
                    if tagCount=1 then w[0]:=word(tagValue)
                    else
                    if tagCount=2 then
                    begin
                      w[0]:=word(tagValue);
                    ...
                    begin
                      setLength(dum,tagCount);
                      fileStream.seek(tagValue,soFromBeginning);
                      fileStream.Read(dum[0],2*tagCount);
                      for i:=0 to tagCount-1 do
                        w[i]:=dum[i];
                    end;
                  end;
      TIFF_LONG:  begin
                    if tagCount=1 then w[0]:=tagValue
                    else
                    begin
                      fileStream.seek(tagValue,soFromBeginning);
                      fileStream.Read(w[0],4*tagCount);
                    end;
                  end;
      else messageCentral('readIntArray : type='+Istr(tagType));
    end;
  end;
end;
...
var
  ...
  stkEntry:TUIC2Entry;
begin
  StkCount:=entry.tagCount;
  setLength(stk,stkCount);
  fileStream.seek(entry.tagValue,soFromBeginning);
  for i:=0 to stkCount-1 do
  begin
    fileStream.Read(stkEntry,sizeof(stkEntry));
    with stkEntry do
    begin
      if ZdistanceD>0 then stk[i].Zdistance:=ZdistanceN/ZdistanceD;
      stk[i].creationTime:=creationTime;
      stk[i].creationDate:=creationDate;
      stk[i].modificationTime:=modificationTime;
      stk[i].modificationDate:=modificationDate;
    end;
  end;
end;
...
var
  i,res:integer;
  stkEntry:array[1..2] of longWord;
begin
  setLength(stk,entry.tagCount);
  fileStream.seek(entry.tagValue,soFromBeginning);
  for i:=0 to entry.tagCount-1 do
  begin
    fileStream.Read(stkEntry,sizeof(stkEntry));
    if stkEntry[2]>0 then stk[i].waveLength:= stkEntry[1]/stkEntry[2];
  end;
end;
...
begin
  case entry.tag of
    TIFFTAG_PHOTOMETRIC :     BlackIsZero:= (tagInt=1);
    TIFFTAG_COMPRESSION :     compression:= tagInt;
    TIFFTAG_IMAGEWIDTH  :     ImageWidth:=tagInt;
    TIFFTAG_IMAGELENGTH :     ImageHeight:=tagInt;
    TIFFTAG_RESOLUTIONUNIT :  ResolutionUnit:=tagInt;
    TIFFTAG_XRESOLUTION :     Xresolution:=tagRational;
    TIFFTAG_YRESOLUTION :     Yresolution:=tagRational;

    TIFFTAG_BITSPERSAMPLE :   readIntArray(BitsPerSample);
    TIFFTAG_SAMPLESPERPIXEL:  SamplesPerPixel:=tagInt;

    TIFFTAG_ROWSPERSTRIP :    RowsPerStrip:=tagInt;
    TIFFTAG_STRIPBYTECOUNTS : ReadIntArray(StripByteCounts);
    TIFFTAG_STRIPOFFSETS :    ReadIntArray(StripOffsets);
...
function TstkFile.Init(stF: AnsiString):boolean;
var
  header:TtiffHeader;
  res:integer;
  i:integer;
...
begin
  ...
  Fintel:= (header.ByteOrder=$4949);

  if header.sgn<>42 then exit;

  fileStream.seek(header.FirstIFD,soFromBeginning);
  fileStream.read(nb,sizeof(nb));

  setLength(DirEntry,nb);
  fileStream.read(DirEntry[0],nb*sizeof(TdirEntry));

  for i:=0 to high(DirEntry) do
    AnalyzeEntry(DirEntry[i]);

  result:=true;


  if RowsPerStrip>0
    then StripCount:=(ImageHeight+RowsPerStrip-1) div RowsPerStrip
    else StripCount:=0;


  FrameSize:=0;
  for i:=0 to high(stripByteCounts) do
    FrameSize:=FrameSize+stripByteCounts[i];

  if length(stripOffsets)>0 then
  begin
    DiskFrameSize:=StripOffsets[high(StripOffsets)]+stripByteCounts[high(StripOffsets)]
                  -StripOffsets[0];
  end;
end;


function TstkFile.loadBuffer(num:integer;var bufsize:integer): pointer;
var
  i:integer;
  offset:integer;
begin
  ...
  begin
    fileStream.seek(StripOffsets[i]+num*DiskFrameSize,soFromBeginning);
    fileStream.Read(PtabOctet(result)^[offset],stripByteCounts[i]);
    offset:=offset+stripByteCounts[i];
  end;                            
end;

function TstkFile.getFileStream: TfileStream;
begin
  if not assigned(file0) then
    try
      file0:=TfileStream.create(stfile,fmOpenRead);
    except
      file0:=nil;
    end;
...
procedure TstkFile.FreeFileStream;
begin
  file0.free;
  file0:=nil;
end;
