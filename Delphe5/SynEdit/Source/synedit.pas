...
{$ENDIF}
  ...
  THookedCommandEvent = procedure(Sender: TObject; AfterProcessing: boolean;
    var Handled: boolean; var Command: TSynEditorCommand; var AChar: AnsiChar;
  ...
  TProcessCommandEvent = procedure(Sender: TObject;
    var Command: TSynEditorCommand; var AChar: AnsiChar; Data: pointer) of object;

  TReplaceTextEvent = procedure(Sender: TObject; const ASearch, AReplace:
    AnsiString; Line, Column: integer; var Action: TSynReplaceAction) of object;
  ...
  TGutterGetTextEvent = procedure(Sender: TObject; aLine: integer;
    var aText: AnsiString) of object;
...
// use scAll to update a statusbar when another TCustomSynEdit got the focus
  ...
  TContextHelpEvent = procedure(Sender: TObject; word : AnsiString)
    of object;
...
{$ENDIF}
    ...
    function GetLineText: AnsiString;
    ...
    function SynGetText: AnsiString;
    function GetWordAtCursor : AnsiString;
    function GetWordAtMouse: AnsiString;
    function GetWordWrap: boolean;
    procedure GutterChanged(Sender: TObject);
    procedure InsertBlock(const BB, BE: TBufferCoord; ChangeStr: PChar; AddToUndoList: Boolean);
    function LeftSpaces(const Line: AnsiString): Integer;
    function LeftSpacesEx(const Line: AnsiString; WantTabs: Boolean): Integer;
    function GetLeftSpacing(CharCount: Integer; WantTabs: Boolean): AnsiString;
    ...
    procedure SetLineText(Value: AnsiString);
    ...
    procedure SetSelTextExternal(const Value: AnsiString);
    procedure SetTabWidth(Value: integer);
    procedure SynSetText(const Value: AnsiString);
    procedure SetTopLine(Value: Integer);
    procedure SetWordWrap(const Value: boolean);
    procedure SetWordWrapGlyph(const Value: TSynGlyph);
    procedure WordWrapGlyphChange(Sender: TObject);
    procedure SizeOrFontChanged(bFont: boolean);
    procedure ProperSetLine(ALine: integer; const ALineText: AnsiString);
...
{$IFDEF SYN_MBCSSUPPORT}
    procedure MBCSGetSelRangeInLineWhenColumnSelectionMode(const s: AnsiString;
...
{$ENDIF}
    ...
    procedure NotifyHookedCommandHandlers(AfterProcessing: boolean;
      var Command: TSynEditorCommand; var AChar: AnsiChar; Data: pointer); virtual;
    ...
    procedure SetSelTextPrimitive(const aValue: AnsiString);
  ...
  protected
    ...
    procedure DoOnCommandProcessed(Command: TSynEditorCommand; AChar: AnsiChar;
    ...
    procedure DoOnProcessCommand(var Command: TSynEditorCommand;
      var AChar: AnsiChar; Data: pointer); virtual;
    function DoOnReplaceText(const ASearch, AReplace: AnsiString;
...
{$ENDIF}
    ...
    procedure CommandProcessor(Command: TSynEditorCommand; AChar: AnsiChar;
      Data: pointer); virtual;
    procedure ClearUndo;
    procedure CopyToClipboard;
    procedure CutToClipboard;
    procedure DoCopyToClipboard(const SText: AnsiString);
...
{$ENDIF}
    procedure ExecuteCommand(Command: TSynEditorCommand; AChar: AnsiChar;
      Data: pointer); virtual;
    function GetBookMark(BookMark: integer; var X, Y: integer): boolean;
    function GetHighlighterAttriAtRowCol(const XY: TBufferCoord; var Token: AnsiString;
      var Attri: TSynHighlighterAttributes): boolean;
    function GetHighlighterAttriAtRowColEx(const XY: TBufferCoord; var Token: AnsiString;
      var TokenType, Start: Integer;
      var Attri: TSynHighlighterAttributes): boolean;
    function GetPositionOfMouse(out aPos: TBufferCoord): Boolean;
    function GetWordAtRowCol(const XY: TBufferCoord): AnsiString;
    ...
    function SearchReplace(const ASearch, AReplace: AnsiString;
...
{$ENDIF}
  ...
  public
    ...
    property SelText: AnsiString read GetSelText write SetSelTextExternal;
    property StateFlags: TSynStateFlags read fStateFlags;
    property Text: AnsiString read SynGetText write SynSetText;
    property TopLine: Integer read fTopLine write SetTopLine;
    property WordAtCursor: AnsiString read GetWordAtCursor;
    property WordAtMouse: AnsiString read GetWordAtMouse;
...
function TrimTrailingSpaces(const S: AnsiString): AnsiString;
var
  I: Integer;
begin
  I := Length(S);
...
function StringIsBlank(const S: AnsiString): Boolean;
var
  i: Integer;
begin
...
procedure TCustomSynEdit.DoCopyToClipboard(const SText: AnsiString);
{$IFDEF SYN_CLX}
begin
...
function TCustomSynEdit.GetLineText: AnsiString;
begin
...
function TCustomSynEdit.GetSelText: AnsiString;

  function CopyPadded(const S: AnsiString; Index, Count: integer): AnsiString;
  var
    SrcLen: Integer;
    DstLen: integer;
    P: PChar;
  ...
  procedure CopyAndForward(const S: AnsiString; Index, Count: Integer; var P:
    PChar);
  var
    pSrc: PChar;
    SrcLen: Integer;
  ...
  function CopyPaddedAndForward(const S: AnsiString; Index, Count: Integer;
    var P: PChar): Integer;
    ...
    case fActiveSelectionMode of
      smNormal:
        if (First = Last) then
          ...
          // step1: calculate total length of result AnsiString
          ...
          // step2: build up result AnsiString
      ...
      smColumn:
        begin
          ...
          // step1: pre-allocate AnsiString large enough for worst case
...
{$ENDIF}
          SetLength(Result, TotalLen);
          P := PChar(Result);
          // step2: copy chunks to the pre-allocated AnsiString
          TotalLen := 0;
...
function TCustomSynEdit.SynGetText: AnsiString;
begin
  Result := Lines.Text;
end;

function TCustomSynEdit.GetWordAtCursor : AnsiString;
begin
...
function TCustomSynEdit.LeftSpaces(const Line: AnsiString): Integer;
begin
  Result := LeftSpacesEx(Line,false);
end;

function TCustomSynEdit.LeftSpacesEx(const Line: AnsiString; WantTabs: Boolean): Integer;
var
  p: PChar;
begin
  p := pointer(Line);
...
function TCustomSynEdit.GetLeftSpacing(CharCount: Integer; WantTabs: Boolean): AnsiString;
begin
...
var
  ...
  TokenAccu: record
    // Note: s is not managed as a AnsiString, it will only grow!!!
    // Never use AppendStr or "+", use Len and MaxLen instead and
    // copy the AnsiString chars directly. This is for efficiency.
...
{$ENDIF}
  ...
  var
    ...
    cRow: integer;
    sLine: AnsiString; // the current line (expanded)
    sToken: AnsiString; // highlighter token info
  ...
  begin
    ...
    // Make sure the token accumulator AnsiString doesn't get reassigned too often.
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            // first AnsiChar in a row and PaintToken will do the actual clipping
            vAuxPos.Column := 1;
          end;
...
procedure TCustomSynEdit.SetLineText(Value: AnsiString);
begin
...
procedure TCustomSynEdit.SetSelTextPrimitive(const aValue: AnsiString);
begin
    ...
    case fActiveSelectionMode of
      smNormal:
        begin
          ...
          begin
              // Create a AnsiString that contains everything on the first line up
...
{$ENDIF}
  ...
  procedure InsertText;
    ...
    var
      sLeftSide: AnsiString;
      sRightSide: AnsiString;
      Str: AnsiString;
      Start: PChar;
      P: PChar;
    ...
    begin
      Result := 0;
      // Insert AnsiString at current position
      InsertPos := CaretX;
      Start := PChar(Value);
      repeat
        P := GetEOL(Start);
...
{$ENDIF}
    ...
    begin
      ...
      // Insert AnsiString before current line
      Start := PChar(Value);
      repeat
        P := GetEOL(Start);
        if P <> Start then
...
procedure TCustomSynEdit.SynSetText(const Value: AnsiString);
begin
  Lines.Text := Value;
end;
...
var
  ...
  szPathName: array[0..260] of AnsiChar;
  Point: TPoint;
...
begin
  ...
  case Msg.ScrollCode of
    ...
    SB_BOTTOM:
      ...
      // Scrolls one AnsiChar left / right
...
procedure TCustomSynEdit.ExecuteCommand(Command: TSynEditorCommand; AChar: AnsiChar;
  Data: pointer);
...
procedure TCustomSynEdit.DoOnProcessCommand(var Command: TSynEditorCommand;
  var AChar: AnsiChar; Data: pointer);
begin
...
begin
  ...
  begin
    ...
    if CX >= LineLen then
    begin
      // find first IdentChar or multibyte AnsiChar in the next line
...
{$IFDEF SYN_MBCSSUPPORT}
      ...
      // find next "whitespace" if current AnsiChar is an IdentChar
...
begin
  ...
  begin
    ...
    if CX > 1 then
    begin  // only find previous AnsiChar, if not already on start of line
      // if previous AnsiChar isn't a "whitespace" search for the last IdentChar
...
{$IFDEF SYN_MBCSSUPPORT}
      ...
      // if previous AnsiChar is a "whitespace" search for the last IdentChar
...
procedure TCustomSynEdit.SetSelTextExternal(const Value: AnsiString);
var
...
function TCustomSynEdit.SearchReplace(const ASearch, AReplace: AnsiString;
  AOptions: TSynSearchOptions): integer;
var
...
begin
  ...
  Result := 0;
  // can't search for or replace an empty AnsiString
...
procedure TCustomSynEdit.MBCSGetSelRangeInLineWhenColumnSelectionMode(
  const s: AnsiString; var ColFrom, ColTo: Integer);
...
begin
  ...
  s := LineText;
  nLineLen := Length(s);
  // only moving or selecting one AnsiChar can change the line
  ...
  begin
    ...
    // don't go past last AnsiChar when ScrollPastEol option not set
    if (DX > 0) and bChangeY then
      ptDst.Char := Min(ptDst.Char, nLineLen + 1);
{$IFDEF SYN_MBCSSUPPORT}
    // prevent from getting inside of a doublebyte AnsiChar
...
procedure TCustomSynEdit.DoCaseChange(const Cmd : TSynEditorCommand);
  ...
  function ToggleCase(const aStr : AnsiString) : AnsiString;
  var
    i : Integer;
    s1, s2 : AnsiString;
  begin
...
var
  ...
  PrevLine, OldSelText : AnsiString;
  p: PChar;
...
begin
  ...
  try
    ...
    begin
      ...
      //               chars per line * lines-1    + last line + null AnsiChar
      ...
      //         #9#13#10 * lines-1 + (last line's #9 + null AnsiChar)
      StrToInsert := StrAlloc(InsertStrLen);
      Run := StrToInsert;
      Spaces := #9;
    end;
...
var
  ...
  begin
    ...
    //spaces, yet we need to delete the whole tab width even though the AnsiChar
...
const
  Brackets: array[0..7] of AnsiChar = ('(', ')', '[', ']', '{', '}', '<', '>');
var
  Line: AnsiString;
  i, PosX, PosY, Len: integer;
  Test, BracketInc, BracketDec: AnsiChar;
  NumBrackets: integer;
  vDummy: AnsiString;
  attr:TSynHighlighterAttributes;
  p: TBufferCoord;
...
function TCustomSynEdit.GetHighlighterAttriAtRowCol(const XY: TBufferCoord;
  var Token: AnsiString; var Attri: TSynHighlighterAttributes): boolean;
...
function TCustomSynEdit.GetHighlighterAttriAtRowColEx(const XY: TBufferCoord;
  var Token: AnsiString; var TokenType, Start: Integer;
  var Attri: TSynHighlighterAttributes): boolean;
var
  PosX, PosY: integer;
  Line: AnsiString;
...
procedure TCustomSynEdit.NotifyHookedCommandHandlers(AfterProcessing: boolean;
  var Command: TSynEditorCommand; var AChar: AnsiChar; Data: pointer);
var
  Handled: boolean;
  i: integer;
  Entry: THookedCommandHandlerEntry;
...
function TCustomSynEdit.DoOnReplaceText(const ASearch, AReplace: AnsiString;
...
function TCustomSynEdit.GetWordAtRowCol(const XY: TBufferCoord): AnsiString;
var
  v_WordStart: TBufferCoord;
  v_WordEnd: TBufferCoord;
begin
...
procedure TCustomSynEdit.ProperSetLine(ALine: integer; const ALineText: AnsiString);
begin
  if eoTrimTrailingSpaces in Options then
...
function TCustomSynEdit.GetWordAtMouse: AnsiString;
var
  Point: TBufferCoord;
begin
