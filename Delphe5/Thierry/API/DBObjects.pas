...
{fonctions héritées}
procedure proTDBObject_ImplicitValue(st:string;var x:TGvariant;var pu:typeUO);pascal;
function fonctionTDBObject_ImplicitValue(st:string;var pu:typeUO):TGvariant;pascal;
procedure proTDBObject_AddField(st:string;tp:integer;var pu:typeUO);pascal;
procedure proTDBObject_DeleteField(st:string;var pu:typeUO);pascal;
procedure proTDBObject_clear(var pu:typeUO);pascal;
function fonctionTDBObject_FieldExists(st:string;var pu:typeUO):boolean;pascal;
...
function fonctionTDBObject_Names(n:integer;var pu:typeUO):string;pascal;
function fonctionTDBObject_ValString(n:integer;var pu:typeUO):string;pascal;
...
implementation
    ...
    function TDBObject.getPk:String;
        begin

        end;

    function TDBObject.default_where:String;
        var
    ...
    procedure TDBObject.remove(commit:Boolean=True);
        var
            statement:IZStatement;
            query,where,separator,field:String;
            builder:TQueryBuilder;
            test:String;
    ...
    class function TDBObject.stmClassName:string;
        begin
            result:='DBObject';
        end;
        ...
        begin
            ...
            for i := 1 to  Length(fields) do
                    with fields[i-1] do
                        ...
                        else raise WrongFormatException.Create('please give a list of String, AnsiString or Char');
        end;
    }
    ...
    procedure proTDBObject_ImplicitValue(st:string;var x:TGvariant;var pu:typeUO);
        begin
            verifierObjet(pu);
            TDBObject(pu).setvalue(st,x);
            if TDBObject(pu).Ferror then sortieErreur('TDBObject.ImplicitValue : field does not exists');
        end;

    function fonctionTDBObject_ImplicitValue(st:string;var pu:typeUO):TGvariant;
        begin
            verifierObjet(pu);
    ...
    procedure proTDBObject_AddField(st:string;tp:integer;var pu:typeUO);
        begin
            verifierObjet(pu);
            if (tp<integer(low(TGvariantType))) or (tp>integer(high(TGvariantType)))
    ...
    procedure proTDBObject_DeleteField(st:string;var pu:typeUO);
        begin
            verifierObjet(pu);
            TDBObject(pu).DeleteField(st);
            if TDBObject(pu).Ferror then sortieErreur('TDBObject.DeleteField : field does not exists');
        end;
    ...
    function fonctionTDBObject_FieldExists(st:string;var pu:typeUO):boolean;
        begin
            verifierObjet(pu);
    ...
    function fonctionTDBObject_Names(n:integer;var pu:typeUO):string;
        begin
            verifierObjet(pu);
            with TDBObject(pu) do
            if (n>=1) and (n<=fields.count)
    ...
    function fonctionTDBObject_ValString(n:integer;var pu:typeUO):string;
        begin
            verifierObjet(pu);
            with TDBObject(pu) do
            if (n>=1) and (n<=fields.count)
