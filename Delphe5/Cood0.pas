...
type
  ...
  private
    ...
    visu0:^TVisuInfo;
    BOKtop,BcancelTop, height0: integer;
  ...
  public
    ...
    function Choose(var title:AnsiString;var visu:TVisuInfo;
          cadrerX1,cadrerY1:TnotifyEvent; Const BinFact:boolean = false):boolean;
  end;
...
procedure TCooD.BOKClick(Sender: TObject);
begin
  updateAllVar(self);
end;
...
begin
  ...
  enBinFactor.Visible:= BinFact;
  LbinFactor.Visible:= BinFact;

  delta:=(h2-h1)*ord(not BinFact);
  BOK.Top:=BOKTop-delta;
  Bcancel.Top:=BcancelTop-delta;
  height:=height0-delta;
...
procedure TCooD.FormCreate(Sender: TObject);
begin
  BOKTop:= BOK.top;
  BcancelTop:=Bcancel.Top;
  height0:=height;
  h1:= enCpx.Top + enCpx.Height;
  h2:= enBinFactor.top + enBinFactor.height;
