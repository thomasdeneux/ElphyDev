...
uses
  Windows, SysUtils, Classes, DXConstsG,
  util1, debug0;
...
type
  ...
  public
    ...
    property ImageName: AnsiString read FImageName write FImageName;
...
begin
  Result := Pointer(intG(FTopPBits)+FNextLine*y);
end;
    ...
    case FBitCount of
      1 : begin
            if FPackedPixelOrder then
              Result := (PByte(intG(FTopPBits)+FNextLine*y+x shr 3)^ and Mask1[7-x and 7]) shr Shift1[7-x and 7]
            else
              Result := (PByte(intG(FTopPBits)+FNextLine*y+x shr 3)^ and Mask1[x and 7]) shr Shift1[x and 7];
          end;
      2 : begin
            if FPackedPixelOrder then
              Result := (PByte(intG(FTopPBits)+FNextLine*y+x shr 2)^ and Mask2[3-x and 3]) shr Shift2[3-x and 3]
            else
              Result := (PByte(intG(FTopPBits)+FNextLine*y+x shr 2)^ and Mask2[x and 3]) shr Shift2[x and 3];
          end;
      4 : begin
            if FPackedPixelOrder then
              Result := (PByte(intG(FTopPBits)+FNextLine*y+x shr 1)^ and Mask4[1-x and 1]) shr Shift4[1-x and 1]
            else
              Result := (PByte(intG(FTopPBits)+FNextLine*y+x shr 1)^ and Mask4[x and 1]) shr Shift4[x and 1];
          end;
      8 : Result := PByte(intG(FTopPBits)+FNextLine*y+x)^;
      16: Result := PWord(intG(FTopPBits)+FNextLine*y+x*2)^;
      24: PByte3(@Result)^ := PByte3(intG(FTopPBits)+FNextLine*y+x*3)^;
      32: Result := PDWORD(intG(FTopPBits)+FNextLine*y+x*4)^;
    end;
  end;
end;
...
begin
  ...
  begin
    case FBitCount of
      1 : begin
            P := Pointer(intG(FTopPBits)+FNextLine*y+x shr 3);
            if FPackedPixelOrder then
      ...
      2 : begin
            P := Pointer(intG(FTopPBits)+FNextLine*y+x shr 2);
            if FPackedPixelOrder then
      ...
      4 : begin
            P := Pointer(intG(FTopPBits)+FNextLine*y+x shr 1);
            if FPackedPixelOrder then
              P^ := (P^ and (not Mask4[1-x and 1])) or ((c and 7) shl Shift4[1-x and 1])
            else
              P^ := (P^ and (not Mask4[x and 1])) or ((c and 7) shl Shift4[x and 1]);
          end;
      8 : PByte(intG(FTopPBits)+FNextLine*y+x)^ := c;
      16: PWord(intG(FTopPBits)+FNextLine*y+x*2)^ := c;
      24: PByte3(intG(FTopPBits)+FNextLine*y+x*3)^ := PByte3(@c)^;
      32: PDWORD(intG(FTopPBits)+FNextLine*y+x*4)^ := c;
    end;
  end;
end;

procedure TDXTextureImage.LoadFromFile(const FileName: AnsiString);
var
  Stream: TFileStream;
begin
...
procedure TDXTextureImage.SaveToFile(const FileName: AnsiString);
var
  Stream: TFileStream;
begin
      ...
      begin
        ...
        begin
          case B2 of
            0: begin  {  End of line  }
                 X := 0; Inc(Y);
                 Dest := Pointer(intG(Image.TopPBits)+Y*Image.NextLine+X);
               end;
            1: Break; {  End of bitmap  }
            2: begin  {  Difference of coordinates  }
                 Inc(X, B1); Inc(Y, B2); Inc(Src, 2);
...
Initialization
AffDebug('Initialization DXTexImgG',0);
finalization
