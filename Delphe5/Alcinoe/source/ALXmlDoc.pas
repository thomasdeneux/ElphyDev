...
type
  ...
  TALXmlNodeType = (
                    ...
                    ntCData,            //[not implemented yet !] The node represents a CDATA section in the XML source. CDATA sections identify blocks of text that would otherwise be interpreted as markup. An ntCData node can’t have any child nodes. They can appear as the child of an ntDocFragment, ntEntityRef, or ntElement node.
  ...
  protected
    ...
    procedure SaveToStream(const Stream: TStream); // [added from TXMLNode]
  ...
  protected
    ...
    Procedure InternalParseXml(RawXmlStream: TStream); overload; // [added from TXMLDocument]
...
Var RawXmlString: TALXMLString;
  ...
  Begin
    ...
    If (str1<>'') and
       (
        (PreserveWhiteSpace and (currentNode.NodeType <> ntdocument)) or
        (Trim(Str1) <> '')
       )
    then Begin
...
Begin
  ...
  Try
    ...
    CurrentNode := FDocumentNode;
    NotSaxMode := assigned(CurrentNode);
    ...
    While RawXmlStringPos <= RawXmlStringLength do begin
      ...
      if RawXmlStringPos + 2 >= RawXmlStringLength then ExpandRawXmlString;
...
procedure TALXMLNode.SaveToStream(const Stream: TStream);
    ...
    While NodeStack.Count > 0 Do begin
      ...
      with CurrentNode do
        case NodeType of
          ...
          ntDocument:	       WriteDocumentNode2Stream(CurrentNode);
          ntDocType:         ;//todo
          ntDocFragment:	   ;//todo
          ntNotation:	       ;//todo
        end;
