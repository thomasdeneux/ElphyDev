...
type
  ...
  private
    FWinInetHttpClient: TalWinHttpClient;
...
begin
  FWinInetHttpClient := TaLWinHttpClient.Create(self);
  with FWinInetHttpClient do begin
...
Var AHTTPResponseHeader: TALHTTPResponseHeader;
    AHTTPResponseStream: TStringStream;
    AMultiPartFormDataFile: TALMultiPartFormDataFile;
    AMultiPartFormDataFiles: TALMultiPartFormDataFiles;
    i: Integer;
begin
  initWinInetHTTP;
  MemoContentBody.Lines.Clear;
  ...
  AHTTPResponseHeader := TALHTTPResponseHeader.Create;
  AHTTPResponseStream := TstringStream.Create('');
  AMultiPartFormDataFiles := TALMultiPartFormDataFiles.Create(true);
  try
    Try
      For I := 0 To MemoPostDataFiles.Lines.Count - 1 do
        if MemoPostDataFiles.Lines[i] <> '' then begin
          AMultiPartFormDataFile := TALMultiPartFormDataFile.Create;
          TmemoryStream(AMultiPartFormDataFile.FileData).LoadFromFile(MemoPostDataFiles.Lines.ValueFromIndex[i]);
          AMultiPartFormDataFile.FileName := ExtractFileName(MemoPostDataFiles.Lines.ValueFromIndex[i]);
          ...
          AMultiPartFormDataFile.FieldName := MemoPostDataFiles.Lines.Names[i];
