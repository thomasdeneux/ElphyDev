...
Var aSmtpClientHeader: TALSmtpClientHeader;
    AMultiPartMixedAttachments : TALMultiPartMixedAttachments;
    i : integer;
    Str: String;
begin
  aSmtpClientHeader := TALSmtpClientHeader.Create;
  Try

    aSmtpClientHeader.From := FromEdit.Text;
    aSmtpClientHeader.SendTo := ToEdit.Text;;
    aSmtpClientHeader.cc := ccEdit.Text;;
    aSmtpClientHeader.Subject := SubjectEdit.Text;
    If ConfirmCheckBox.Checked then aSmtpClientHeader.DispositionNotificationTo := FromEdit.Text;
    aSmtpClientHeader.Priority := PriorityComboBox.Text;
    If PriorityComboBox.ItemIndex = 1 then str := 'High'
    else If PriorityComboBox.ItemIndex = 2 then str := 'Normal'
    else str := 'Low';
    aSmtpClientHeader.CustomHeaders.Add('X-MSMail-Priority: ' + str);
    If trim(FileAttachMemo.Lines.text) <> '' then begin
      AMultiPartMixedAttachments := TALMultiPartMixedAttachments.Create(true);
      Try
        For i := 0 to FileAttachMemo.Lines.Count - 1 do
          If FileAttachMemo.Lines[i] <> '' then
            AMultiPartMixedAttachments.Add.LoadFromFile(trim(FileAttachMemo.Lines[i]));
          DisplayMemo.Lines.Add(trim(FSmtpClient.DataMultipartMixed(aSmtpClientHeader, MsgMemo.Lines.Text, 'text/plain', AMultiPartMixedAttachments)));
      finally
        AMultiPartMixedAttachments.Free;
      end;
    end
    else DisplayMemo.Lines.Add(trim(FSmtpClient.Data(aSmtpClientHeader, MsgMemo.Lines.Text)));


  finally
    aSmtpClientHeader.Free;
  end;
end;
    ...
    If trim(FileAttachMemo.Lines.text) <> '' then begin
      ...
      Try
        ...
        AMultiPartMixedAttachments.Free;
                              ...
                              PasswordEdit.Text,
                              TAlSmtpClientAuthType(AuthComboBox.ItemIndex),
                              aSmtpClientHeader.RawHeaderText,
                              MsgMemo.Lines.Text
                              );
