...
const
  ElphySpkPacketFixedSize=sizeof(TElphySpkPacket)-128*2;         // =24
...
function InitCyberKDLL:boolean;overload;
function InitCyberKDLL(stdll:string):boolean;overload;
...
function InitCyberKDLL(stdll:string):boolean;
begin
  result:=true;
  if (hh<>0) and (stDll=DllName) then exit;

  if hh<>0 then FreeLibrary(hh);

  hh:=GloadLibrary(stDll);
  result:=(hh<>0);
