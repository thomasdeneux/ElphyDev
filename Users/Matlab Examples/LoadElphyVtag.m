...
function [V,SampInt] = LoadElphyVtag(stName, NumEp, NumTag)
% V = LoadElphyAnalog(FileName, NumEpisode, NumTag)
% Open the Elphy datafile with name FileName and load the vector corresponding
% to episode NumEpisode and tag channel NumTag
  ...
  try
    ...
    V = bitshift(V,-NumTag+1);
    V = bitand(V,1);
    SampInt=Dxu*Ksamp(NumChan);
